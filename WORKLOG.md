...
### 2026-06-15 — 제품 리스트 할인카드 '정상가'/'제휴카드가' 라벨 분리
**커밋:** `365521b` (feat) → logscript `@365521b` 배포

**요청:** 할인 카드의 취소선 가격(정가)이 무슨 가격인지 라벨이 없어 헷갈림 → 할인전 가격을
명확히 라벨링하고 싶다.

**기존 `bjpFormat`(prod_list):** 할인 카드 상단 라벨 `제휴카드가` + 우측 취소선(정가), 그 아래
파란 할인가. → 정작 `제휴카드가` 라벨이 가리키는 건 취소선(정가)이 아니라 아래 파란 할인가라
의미가 어긋남.

**변경:**
- 할인 카드: 상단 라벨 `제휴카드가`→`정상가`(취소선 정가 라벨), 그 아래 점선 구분 +
  `제휴카드가` 캡션을 새로 넣어 파란 할인가를 직접 라벨링. → 정상가(할인전)/제휴카드가(할인후) 명확.
- 신규 `.bj-pz-cap`(12px 회색, `border-top:1px dashed` 구분선).
- **정렬 유지:** 캡션 1행이 늘어 단일가 카드와 높이가 어긋남 → 단일 카드에도 동일 캡션을
  숨김 placeholder(`.bj-pz-cap.bj-pz-ph`, 바닥에 배치)로 넣어 높이 맞춤.

**검증:** route-interception으로 로컬 inject.js를 라이브 prod_list/1-8에 주입해 캡처
(`deploy/verify-cap.js`). 할인 카드 box 537px vs 단일 카드 541px(4px 차, 사실상 정렬). 육안 OK 후 배포.
