...
**⚠️ 롤백됨(2026-07-02):** 배포 직후 Jun 환경에서 사이트가 원본(주입 실패)으로 표시 — 추가요소 안 보이고 숨긴 섹션 다시 노출. 원인 추정 = jsDelivr 신규 해시(d37c796) 전파 지연으로 일부 엣지 404 → 스크립트 로드 실패. (헤드리스 Playwright·curl에선 d37c796 정상 200·전 요소 렌더 확인, 콘솔 에러 없음.) Jun 강력새로고침에도 미표시 → 요청대로 **logscript `@195c998`로 롤백 배포**(정상 확인됨: bars=2, marquee, trustSub). d37c796 코드(eyebrow 제거 + 신뢰도 시안 카피)는 HEAD에 유지, **CDN 완전 전파 후 재배포 예정**. 재배포 시 push 직후 CDN URL 워밍업(200 확인) 후 logscript 갱신 순서 권장.
