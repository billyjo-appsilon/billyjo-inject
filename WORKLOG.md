...
**밀린 물량 처리 — 실전 회차로 한계 측정 후 조정**
1회차(상한 300)를 수동 실행한 결과: `catalog 6,914 / existing 3,908 / new 3,574 / this run 300`
→ **Saved 138 · Skipped 162**, 실패는 **전부 `page.goto` 타임아웃**이었다. 사이트가 한 창에
대략 140건 언저리에서 막는다(로컬 실측도 순차 1건/1초에 143건 차단). 한 회차에 더 밀어넣으면
실패만 쌓이고 사이트 부담만 는다.
→ 상한 **300 → 150**, 워크플로 **하루 1회 → 3시간마다**(public 레포라 Actions 과금 없음).
하루 약 1,100건 처리 → 밀린 3,274건을 **3~4일**에 소화. 신규 등록분은 **최대 3시간**이면
카드가 생긴다(기존엔 다음 날 새벽까지 대기). 실패분은 다음 회차가 자동 재시도.

**엔드투엔드 검증** (1회차 반영 후 라이브)
| 상품 | 상태 |
|---|---|
| `35198` 삼성 BESPOKE (사이트 최신, 어제까지 카드 없음) | AI 카드 ✓(섹션 5) · 추천 4장 ✓ · 후기 ✓ |
| `34686` 현대큐밍 벽걸이 에어컨 (이번 회차 생성) | AI 카드 ✓ · 추천 4장 ✓ · 후기 ✓ |

인덱스도 2,958 → **2,992** 로 갱신되어 커밋됐다(`auto: daily card sync … index 2992`).
...
추천에 노출할지는 정책 판단이라 손대지 않았다. → 아래에서 해결.

---

## 2026-07-29 — 브랜드 유추 폴백 정확도 측정 후 적용 (인덱스 누락 1,054건 복구)

**측정 방법**: 정답이 있는 데이터(스펙표 '브랜드' 행이 있어 이미 인덱스에 들어간 제품) 60건을
무작위로 뽑아, 후보 규칙이 그 정답을 얼마나 맞히는지 실측.

| 규칙 | 커버 | 정확도 |
|---|---|---|
| 상품명 어휘 매칭 | 76% | 65% |
| 첫 토큰 = 브랜드 | 80% | 60% |
| 공급사 `data-supname` | 80% | 52% |
| **결합(어휘+표기 정규화)** | 76% | **95%** |
| 확장 결합(첫 토큰까지) | 80% | 91% |

단독 규칙은 전부 60% 안팎이라 못 쓴다. 결정적 단서는 **어휘매칭 오답 16건이 전부 공급사 값이
정답**이었다는 것(LG→LG구독 14, 청호→청호나이스 2). 등급별로는 `mid`(공급사가 판매대행사라
상품명을 택한 경우) 19건이 **100%** 정확했다. 남은 오차도 표기 흔들림(SI PAY↔에스아이페이)이거나
원본 자체가 갈리는 경우(동일 신호에 LG 2건 / LG구독 14건)라, LG↔LG구독은 유추하지 않는다.

**적용 (billyjo-cards `66b92a1`)**
- 브랜드 폴백: 어휘매칭 → 첫 토큰 → 별칭 정규화. **1,054건 전부 복구, 유추 실패 0**.
- **결합상품 211건은 제외하지 않고 실제 제품 기준으로 처리**(대표님 지침) —
  `(보람피플 올인원598 1구좌) 쿠쿠 에코웨일 …` → 브랜드 `쿠쿠` · 품목 `식기세척기` ·
  표시명은 괄호 뒤 실제 제품명.

**작업 중 발견한 더 큰 문제 — 카테고리 오염**
`.meta` 캡처가 탐욕적이라 `</div>` 를 넘어 카드 평가표까지 삼켰고, 거기 **"정수성능"** 문구
때문에 수입차·캠핑용품까지 `category='정수기'` 로 찍혔다(**정수기 229 → 1,209**). 매일 인덱스를
재생성하도록 바꾼 직후라 그대로 두면 운영에 그대로 나갈 뻔했다.
→ 캡처를 첫 `</div>` 에서 끊고, 분류 근거도 카드 본문 대신 **구조화 필드**(실제 제품명·종류·
타입·방식)만 쓴다. 수입차·캠핑용품은 null 로 빠지고, 결합상품은 식기세척기로 잡힌다.

**카테고리 보강을 워크플로에 편입**: 발행본의 미분류가 5건뿐이었던 건 빌드 후 사람이
`enrich-categories.js` 를 돌렸기 때문이다. 워크플로엔 그 단계가 없어 재생성하면 미분류가
2,078건으로 늘었다 → 단계 추가. 이 스크립트도 상품마다 prod_view 를 열던 방식(수천 요청, 406
차단)에서 **목록 ajax(126요청)** 로 전환. 결과 미분류 **2,078 → 255**.

**부수 정리**: 소비처가 없는데 파일의 70%를 차지하던 `productNameMeta` 를 출력에서 제외
(admin2 가 추천마다 받는 파일이다). **13.7MB → 4.5MB**.

**결과 검증**
- 인덱스 2,992 → **4,046**건
- `image_status` 재수집: 추천가능 2,195 → **3,478** · 제외 568(등록 미완 351)
- **24578(코웨이 얼음정수기)은 그동안 추천 0장이었는데 3장 확보** — 후보 풀이 좁아서였다
- 무작위 12개 상품 회귀: 카드 수 변화 없음 · 라이브 카나리 22장 전부 이미지 정상

---

## 2026-07-29 — 추가·삭제·가격까지 일단위 반영 (빠져 있던 세 번째 고리)

**현재 자동화 상태**

| 무엇 | 주기 | 관리 위치 |
|---|---|---|
| 상품 추가 감지 → 카드 생성 → 인덱스 재생성 → 카테고리 보강 | 3시간 | GitHub Actions(billyjo-cards) |
| 상품 삭제(목록 이탈) 감지 → 추천 후보 제외 | 매일 04:30 | admin2 `/collections` |
| **가격 변동 반영** | 매일 04:30(같은 스윕) | admin2 `/collections` |
| 추천 반영 — 인덱스 10분 TTL · 제외/썸네일/가격 30분 TTL | 상시 | 자동 |
| DB 카탈로그 미러(sync-catalog) | 매일 03:00 KST | Vercel cron |
| 화면 카나리 | 주 1회 | admin2 `/collections` |

**빠져 있던 고리 — 가격**: 추가·삭제는 잡히는데 가격 변동은 아무도 안 봤다.
`daily-sync.js` 는 **신규 상품만** 카드를 만들고 기존 카드는 갱신하지 않아서, 인덱스의
`monthlyFee` 가 카드 생성 시점에 굳는다.

실측(3개 카테고리 278건 대조): **일치 218 · 불일치 60(21%)**.
```
26316 웰스 냉온정수기  32,900 → 43,900  (+11,000)
26315 웰스 냉온정수기  35,900 → 46,900  (+11,000)
```
카탈로그 전체로는 **504건**이 어긋나 있었다. 가격은 추천의 핵심 조건이라(±25% 밴드,
'월 N원 저렴' 배지) 낡은 값을 쓰면 **더 비싼 제품을 싸다고 추천**하게 된다.

**조치 (admin2 `52e1946`)**: 매일 도는 목록 스윕이 이미 126요청으로 전 상품을 훑으므로
같은 응답에서 가격도 함께 걷는다(**추가 요청 0회**). 백엔드는 cards-index 로드 직후
`_apply_fresh_prices()` 로 덮어써서, 스코어링·배지·대체 풀이 자동으로 최신가를 쓴다.
