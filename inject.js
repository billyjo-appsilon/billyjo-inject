...
(function(){
  ...
  function todayKey(){
    try {
      var d = new Date();
      var z = new Date(d.getTime() + 9 * 60 * 60 * 1000);
  ...
  function readCachedCounts(){
    try {
      var raw = localStorage.getItem(RV_CACHE_KEY);
      if(!raw) return null;
      var j = JSON.parse(raw);
      if(j && j.date === todayKey() && normalizeCounts(j.counts)) return j.counts;
    } catch(e) {}
    return null;
  }
  function writeCachedCounts(j){
    try {
      localStorage.setItem(RV_CACHE_KEY, JSON.stringify({date: todayKey(), counts: j}));
    } catch(e) {}
  }
  ...
  function fetchCounts(){
    ...
    if(cached) applyCounts(cached, 'local-cache');
    else if(PRELOADED_COUNTS) applyCounts(PRELOADED_COUNTS, 'preloaded-' + PRELOADED_COUNTS_DATE);
    fetch(API+'/counts', {cache:'no-store'}).then(function(r){return r.json();}).then(function(j){
      if(applyCounts(j, 'api')) writeCachedCounts(j);
    }).catch(function(){});
  }
