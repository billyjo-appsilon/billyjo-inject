...
/* =========================================================================
 * [파비콘] inject.js 자신의 CDN src에서 커밋 해시를 추출해 동일 커밋의
 * favicon.png(레포 루트)를 head <link rel=icon>으로 강제 교체.
 * Cafe24 테마가 뒤늦게 파비콘을 다시 심는 경우를 대비해 DOMContentLoaded에 재적용.
 * ========================================================================= */
(function() {
  function bjFaviconUrl() {
    function fromSrc(src) {
      if (src && src.indexOf("billyjo-inject@") !== -1 && /inject\.js/.test(src)) {
        return src.replace(/inject\.js(\?.*)?$/, "favicon.png");
      }
      return null;
    }
    try {
      if (document.currentScript) {
        var u = fromSrc(document.currentScript.src);
        if (u) return u;
      }
    } catch (e) {}
    var scripts = document.getElementsByTagName("script");
    for (var i = 0; i < scripts.length; i++) {
      var u2 = fromSrc(scripts[i].src || "");
      if (u2) return u2;
    }
    return null;
  }
  function bjSetFavicon() {
    var url = bjFaviconUrl();
    if (!url) return;
