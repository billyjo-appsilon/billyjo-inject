...
(function(){
  ...
  function injectStyles(){
    ...
    s.textContent =
      ...
      '#bj-do-fab{position:fixed;right:18px;bottom:92px;z-index:99998;border:0;border-radius:999px;background:'+BLUE+';color:#fff;font:900 13px Pretendard,Arial,sans-serif;padding:13px 17px;box-shadow:0 14px 30px rgba(8,56,248,.32);cursor:pointer}' +
...
function bjHeaderMainInit() {
  ...
  if (true) {  // 전체 페이지 적용 (2026-07-02): 이전엔 /prod_list/ URL만 → 메인페이지 등 .prod_list .item 카드에도 적용. bjpRun은 .prod_list .item만 대상이라 카드 없는 페이지는 no-op.
    ...
    var PRELOADED_PRICE_SNAPSHOT_DATE = '2026-08-24';
    ...
    if (isVehicleOrder) {
      ...
      if (memoArea) {
        memoArea.placeholder =
          '[사은품 지급 안내]\n' +
          "이전 단계에서 안내받으신 '사은품 코드'가 입력되어 있는지 한 번만 확인해주세요.\n\n" +
          '[필수 확인 및 기재 항목]\n' +
