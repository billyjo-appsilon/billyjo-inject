...
(function(){
  ...
  var DIRECT_COUPON_TITLE = '이번달 특가 프로모션 + 결합 추가사은품 BEST';
  ...
  var DIRECT_HERO_BANNER_DESKTOP_URL = directOfferAsset('images/direct-offer-top-banner-left-text.png');
  var DIRECT_HERO_BANNER_MOBILE_URL = directOfferAsset('images/direct-offer-top-banner-left-text.png');
  ...
  function injectStyles(){
    ...
    s.textContent =
      ...
      '#bj-do-head{padding:0;min-height:154px;background:#0838f8 url("'+DIRECT_HERO_BANNER_DESKTOP_URL+'") center/cover no-repeat;position:relative;color:#fff;overflow:hidden}.bj-do-badge{display:none}.bj-do-h{position:absolute!important;width:1px!important;height:1px!important;padding:0!important;margin:-1px!important;overflow:hidden!important;clip:rect(0,0,0,0)!important;white-space:nowrap!important;border:0!important}.bj-do-h-sub{display:inline-block;font-size:calc(1em - 1px);font-weight:900;margin-left:2px}.bj-do-sub{display:none}.bj-do-x{position:absolute;right:14px;top:14px;width:34px;height:34px;border:0;background:rgba(255,255,255,.92);color:'+BLUE+';font-size:22px;line-height:30px;border-radius:50%;cursor:pointer}' +
      ...
      '@media(max-width:720px){#bj-do-back{align-items:flex-end;padding:0;background:rgba(10,16,30,.55)}#bj-do-box{max-width:none;max-height:92vh;border-radius:18px 18px 0 0}.bj-do-layout{display:block}.bj-do-main{padding:14px}.bj-do-side{padding:12px 14px 18px;border-left:0;border-top:1px solid #e2e8f7}#bj-do-head{min-height:124px;background-image:url("'+DIRECT_HERO_BANNER_MOBILE_URL+'");background-position:center center;background-size:cover}.bj-do-card{grid-template-columns:58px minmax(0,1fr) 52px;gap:9px}.bj-do-card img{width:58px;height:58px}.bj-do-review{display:none}.bj-do-memo{min-height:96px}.bj-do-total-v{font-size:24px}#bj-do-jump{position:fixed;left:16px;right:16px;bottom:calc(env(safe-area-inset-bottom,0px) + 14px);z-index:100003;border:0;border-radius:15px;background:linear-gradient(180deg,#2f68ff 0%,#0838f8 54%,#0029c8 100%);color:#fff;min-height:54px;padding:14px 16px;align-items:center;justify-content:center;gap:8px;font:950 15px/1.18 Pretendard,Arial,sans-serif;letter-spacing:0;box-shadow:0 10px 0 #001d99,0 22px 34px rgba(8,56,248,.38),inset 0 1px 0 rgba(255,255,255,.42),inset 0 -2px 0 rgba(0,0,0,.13);cursor:pointer;overflow:hidden;transform:translateY(0);transition:transform .16s ease,box-shadow .16s ease;isolation:isolate}#bj-do-jump:before{content:"";position:absolute;inset:-86% -44%;background:linear-gradient(115deg,transparent 36%,rgba(255,255,255,.46) 49%,transparent 61%);transform:translateX(-74%) rotate(6deg);animation:bjDoCtaShine 3.1s ease-in-out infinite;z-index:-1}#bj-do-jump:active{transform:translateY(6px);box-shadow:0 5px 0 #001d99,0 13px 22px rgba(8,56,248,.28),inset 0 1px 0 rgba(255,255,255,.35),inset 0 -1px 0 rgba(0,0,0,.16)}#bj-do-jump:after{content:"";width:18px;height:18px;flex:0 0 auto;background:url("data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 width=%2724%27 height=%2724%27 viewBox=%270 0 24 24%27 fill=%27none%27 stroke=%27white%27 stroke-width=%273%27 stroke-linecap=%27round%27 stroke-linejoin=%27round%27%3E%3Cpath d=%27M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4%27/%3E%3Cpolyline points=%277 10 12 15 17 10%27/%3E%3Cline x1=%2712%27 y1=%2715%27 x2=%2712%27 y2=%273%27/%3E%3C/svg%3E") center/contain no-repeat}#bj-do-box.bj-do-jump-on #bj-do-jump{display:flex}}' +
  ...
  function syncQuoteJumpButton(box){
    ...
    var scroller = box && box.querySelector && box.querySelector('#bj-do-body');
