...
(function bjQuoteCompareCart(){
  ...
  function relabelCartLanguage(){
    Array.prototype.forEach.call(document.querySelectorAll('button,a,span,strong,p,th,caption,h1,h2,h3'), function(el){
      ...
      else if (t === '선택상품 렌탈') el.textContent = '선택 제품 견적/신청';
    ...
    Array.prototype.forEach.call(document.querySelectorAll('table.cart-list tbody tr, table.order-field.cart-list tbody tr, .cart_list tbody tr, .shop_cart table tbody tr'), function(row){
      ...
      if (row.querySelector('td') && (
        ...
        cells.forEach(function(cell){
          cell.classList.remove('bj-cart-check-cell', 'bj-cart-thumb-cell', 'bj-cart-info-cell', 'bj-cart-meta-cell');
        });
        var thumbCell = cells.find(function(cell){ return cell.querySelector && cell.querySelector('img'); });
        var checkCell = cells.find(function(cell){ return cell.querySelector && cell.querySelector('input[type="checkbox"]'); });
        var infoCell = cells.find(function(cell){
          if (!cell || cell === checkCell || cell === thumbCell) return false;
          return cell.matches && cell.matches('td.prod,.prod,.name,.goods_name,.cart_tit');
        }) || cells.find(function(cell){
          if (!cell || cell === checkCell || cell === thumbCell) return false;
          return /렌탈|상품|모델|월\s*[\d,]+\s*원/.test(text(cell));
        }) || thumbCell;
        if (checkCell) checkCell.classList.add('bj-cart-check-cell');
        if (thumbCell) thumbCell.classList.add('bj-cart-thumb-cell');
        if (infoCell) infoCell.classList.add('bj-cart-info-cell');
        cells.forEach(function(cell){
          if (cell !== checkCell && cell !== thumbCell && cell !== infoCell) cell.classList.add('bj-cart-meta-cell');
        });
      }
    });
  }
  function enhanceCartActionButtons(){
    if (!isCartPage()) return;
    Array.prototype.forEach.call(document.querySelectorAll('button,a,input[type="button"],input[type="submit"]'), function(btn){
      var label = text(btn);
      if (!label) return;
      if (/선택상품\s*(삭제|견적서\s*출력)/.test(label)) {
        btn.classList.add('bj-cart-secondary-action');
      }
      if (/선택(?:한\s*상품|\s*제품)\s*견적\s*\/?\s*신청|전체\s*상품\s*견적\s*\/?\s*신청|선택상품\s*렌탈|전체상품\s*렌탈/.test(label)) {
        btn.classList.add('bj-cart-primary-action');
        var parent = btn.parentElement;
        if (parent) parent.classList.add('bj-cart-primary-action-wrap');
      }
    });
  }
  function enhanceCartPageDescription(){
    if (!isCartPage()) return;
    var desc = document.querySelector('.page_desc');
    if (!desc || desc.getAttribute('data-bj-cart-desc') === '1') return;
    desc.setAttribute('data-bj-cart-desc', '1');
    desc.classList.add('bj-cart-page-desc');
    desc.innerHTML =
  ...
  function injectQuoteStyle(){
    ...
    st.textContent =
      ...
      '.bj-qam-card{width:100%;max-width:430px;max-height:92vh;overflow:auto;background:#fff;border-radius:16px;padding:20px;box-shadow:0 24px 80px rgba(0,0,0,.28);position:relative;color:#172033}' +
    ...
    st.textContent +=
      ...
      'body.bj-quote-cart-page button.plain.btn_large.c1,body.bj-quote-cart-page input.plain.btn_large.c1{border:0!important;border-radius:12px!important;background:linear-gradient(135deg,#0838f8 0%,#0626c0 100%)!important;color:#fff!important;box-shadow:0 12px 28px rgba(8,56,248,.2)!important;font-weight:900!important;letter-spacing:-.01em!important;transition:transform .18s ease,box-shadow .18s ease,filter .18s ease!important}' +
      'body.bj-quote-cart-page button.plain.btn_large.c2:hover,body.bj-quote-cart-page button.plain.btn_large.c1:hover,body.bj-quote-cart-page input.plain.btn_large.c2:hover,body.bj-quote-cart-page input.plain.btn_large.c1:hover{transform:translateY(-1px)!important;filter:saturate(1.05)!important;box-shadow:0 16px 34px rgba(8,56,248,.25)!important}' +
      'body.bj-quote-cart-page .bj-cart-secondary-action{display:inline-flex!important;align-items:center!important;justify-content:center!important;min-height:38px!important;box-sizing:border-box!important;padding:10px 16px!important;border:1px solid #d9dee8!important;border-radius:10px!important;background:#f3f5f8!important;background-image:none!important;color:#6b7280!important;box-shadow:none!important;font-weight:850!important;line-height:1.2!important;text-decoration:none!important;letter-spacing:0!important;vertical-align:middle!important;transition:background .16s ease,border-color .16s ease,color .16s ease!important}' +
      'body.bj-quote-cart-page .bj-cart-secondary-action:hover{background:#e9edf3!important;border-color:#cbd2dd!important;color:#4b5563!important;transform:none!important;box-shadow:none!important;filter:none!important}' +
      'body.bj-quote-cart-page .bj-cart-primary-action{margin-bottom:16px!important}' +
      'body.bj-quote-cart-page .bj-cart-primary-action-wrap{padding-bottom:16px!important}' +
      'body.bj-quote-cart-page .page_desc.bj-cart-page-desc{margin:0 0 22px!important;padding:17px 20px!important;border:1px solid #dfe7f3!important;border-radius:14px!important;background:#f7f9fc!important;box-shadow:0 8px 22px rgba(15,36,72,.06)!important;color:#445064!important;text-align:left!important}' +
      'body.bj-quote-cart-page .page_desc.bj-cart-page-desc p{margin:0!important;padding:0!important;font-size:15px!important;line-height:1.55!important;color:#445064!important;letter-spacing:0!important}' +
      ...
      '@media(max-width:768px){body.bj-quote-cart-page button.plain.btn_large.c2,body.bj-quote-cart-page button.plain.btn_large.c1,body.bj-quote-cart-page input.plain.btn_large.c2,body.bj-quote-cart-page input.plain.btn_large.c1{border-radius:10px!important;font-size:15px!important;line-height:1.2!important}body.bj-quote-cart-page .bj-cart-secondary-action{min-height:36px!important;padding:9px 14px!important;border-radius:9px!important;font-size:13px!important}body.bj-quote-cart-page .bj-cart-primary-action{margin-bottom:14px!important}body.bj-quote-cart-page .bj-cart-primary-action-wrap{padding-bottom:14px!important}body.bj-quote-cart-page .page_desc.bj-cart-page-desc{margin-bottom:18px!important;padding:15px 16px!important;border-radius:13px!important}body.bj-quote-cart-page .page_desc.bj-cart-page-desc p{font-size:14px!important;line-height:1.5!important}body.bj-quote-cart-page .bj-cart-card-row{display:grid!important;grid-template-columns:86px minmax(0,1fr)!important;gap:8px 12px!important;margin:0 0 14px!important;padding:13px!important;background:#fff!important;border:1px solid #d9e2f0!important;border-radius:16px!important;box-shadow:0 10px 28px rgba(15,36,72,.12)!important;position:relative!important}body.bj-quote-cart-page .bj-cart-card-row>td,body.bj-quote-cart-page .bj-cart-card-row>th{display:block!important;width:auto!important;border:0!important;border-radius:0!important;padding:0!important;text-align:left!important;background:transparent!important;min-width:0!important}body.bj-quote-cart-page .bj-cart-card-row>td:first-child,body.bj-quote-cart-page .bj-cart-card-row>th:first-child,body.bj-quote-cart-page .bj-cart-card-row>td:last-child,body.bj-quote-cart-page .bj-cart-card-row>th:last-child{border:0!important;border-radius:0!important}body.bj-quote-cart-page .bj-cart-check-cell{position:absolute!important;left:10px!important;top:10px!important;z-index:2!important;width:auto!important;background:rgba(255,255,255,.82)!important;border-radius:8px!important;padding:2px!important}body.bj-quote-cart-page .bj-cart-thumb-cell{grid-column:1!important;grid-row:1 / span 4!important;align-self:start!important}body.bj-quote-cart-page .bj-cart-thumb-cell img{display:block!important;width:82px!important;height:82px!important;max-width:82px!important;box-sizing:border-box!important;object-fit:contain!important;background:#f3f6fb!important;border:1px solid #e2e8f3!important;border-radius:12px!important;padding:6px!important}body.bj-quote-cart-page .bj-cart-thumb-cell.bj-cart-info-cell{grid-column:1 / -1!important;grid-row:auto!important;display:grid!important;grid-template-columns:86px minmax(0,1fr)!important;gap:0 12px!important;align-items:start!important}body.bj-quote-cart-page .bj-cart-thumb-cell.bj-cart-info-cell img{grid-column:1!important;grid-row:1 / span 4!important}body.bj-quote-cart-page .bj-cart-info-cell{grid-column:2!important;grid-row:1!important;font-size:14px!important;font-weight:900!important;line-height:1.35!important;color:#17253a!important}body.bj-quote-cart-page .bj-cart-info-cell a{color:#17253a!important;font-weight:900!important;line-height:1.35!important}body.bj-quote-cart-page .bj-cart-meta-cell{grid-column:2!important;font-size:12px!important;line-height:1.45!important;color:#5f6b7c!important}body.bj-quote-cart-page .bj-cart-card-row:not(:has(.bj-cart-thumb-cell)){grid-template-columns:1fr!important}body.bj-quote-cart-page .bj-cart-card-row:not(:has(.bj-cart-thumb-cell)) .bj-cart-info-cell,body.bj-quote-cart-page .bj-cart-card-row:not(:has(.bj-cart-thumb-cell)) .bj-cart-meta-cell{grid-column:1!important}body.bj-quote-cart-page .bj-cart-total-row{display:block!important;margin:0 0 12px!important;padding:13px!important;background:#eef4ff!important;border:1px solid #cfdcff!important;border-radius:16px!important;box-shadow:0 10px 26px rgba(8,56,248,.1)!important}body.bj-quote-cart-page .bj-cart-total-row>td,body.bj-quote-cart-page .bj-cart-total-row>th{display:block!important;width:auto!important;border:0!important;border-radius:0!important;padding:4px 0!important;text-align:left!important;background:transparent!important}body.bj-quote-cart-page .bj-cart-total-row>td:first-child,body.bj-quote-cart-page .bj-cart-total-row>th:first-child,body.bj-quote-cart-page .bj-cart-total-row>td:last-child,body.bj-quote-cart-page .bj-cart-total-row>th:last-child{border:0!important;border-radius:0!important}}';
    st.textContent +=
      '@media(max-width:768px){body.bj-quote-cart-page .cart-list,body.bj-quote-cart-page table.order-field.cart-list,body.bj-quote-cart-page .shop_cart table,body.bj-quote-cart-page .cart_list{display:block!important;width:100%!important;max-width:100%!important;border-spacing:0!important;box-sizing:border-box!important}body.bj-quote-cart-page .cart-list tbody,body.bj-quote-cart-page table.order-field.cart-list tbody,body.bj-quote-cart-page .shop_cart table tbody,body.bj-quote-cart-page .cart_list tbody{display:block!important;width:100%!important;box-sizing:border-box!important}body.bj-quote-cart-page .bj-cart-card-row{width:calc(100% - 32px)!important;max-width:calc(100% - 32px)!important;box-sizing:border-box!important;margin:0 auto 14px!important;grid-template-columns:96px minmax(0,1fr)!important;gap:7px 12px!important;padding:13px!important;align-items:start!important}body.bj-quote-cart-page .bj-cart-check-cell{left:11px!important;top:11px!important;line-height:1!important}body.bj-quote-cart-page .bj-cart-thumb-cell{width:96px!important;grid-column:1!important;grid-row:1 / span 6!important}body.bj-quote-cart-page .bj-cart-thumb-cell img{width:92px!important;height:92px!important;max-width:92px!important}body.bj-quote-cart-page .bj-cart-info-cell{grid-column:2!important;grid-row:1!important;width:auto!important;min-width:0!important;font-size:14px!important;line-height:1.34!important}body.bj-quote-cart-page .bj-cart-info-cell .name,body.bj-quote-cart-page .bj-cart-info-cell .brand{position:static!important;display:block!important;float:none!important;margin:0!important;padding:0!important;height:auto!important;min-height:0!important;max-height:none!important;line-height:1.34!important;letter-spacing:0!important;white-space:normal!important;word-break:keep-all!important;overflow-wrap:anywhere!important}body.bj-quote-cart-page .bj-cart-info-cell .brand{margin-top:4px!important;font-size:12px!important;color:#5f6b7c!important;font-weight:800!important}body.bj-quote-cart-page .bj-cart-meta-cell{grid-column:2!important;width:auto!important;min-width:0!important;height:auto!important;min-height:18px!important;line-height:1.42!important;margin:2px 0 0!important;padding:0!important;position:relative!important;float:none!important;clear:none!important;white-space:normal!important;word-break:keep-all!important;overflow-wrap:anywhere!important}body.bj-quote-cart-page .bj-cart-meta-cell:before,body.bj-quote-cart-page .bj-cart-meta-cell:after{content:none!important;display:none!important}body.bj-quote-cart-page .bj-cart-meta-cell *{position:static!important;float:none!important;line-height:1.42!important}body.bj-quote-cart-page .bj-cart-secondary-action{white-space:nowrap!important}body.bj-quote-cart-page .bj-cart-primary-action-wrap{display:inline-block!important;width:calc(50% - 8px)!important;box-sizing:border-box!important;margin:0 0 14px!important;padding:0!important}body.bj-quote-cart-page .bj-cart-primary-action-wrap+.bj-cart-primary-action-wrap{margin-left:8px!important}body.bj-quote-cart-page .bj-cart-primary-action{display:flex!important;align-items:center!important;justify-content:center!important;width:100%!important;min-height:46px!important;margin:0!important;padding:0 8px!important;box-sizing:border-box!important;white-space:normal!important;word-break:keep-all!important}body.bj-quote-cart-page .bj-cart-total-row{width:calc(100% - 32px)!important;max-width:calc(100% - 32px)!important;box-sizing:border-box!important;margin:0 auto 12px!important}}';
    st.textContent +=
      '@media(max-width:768px){body.bj-quote-cart-page ul.btn-col2{display:flex!important;align-items:stretch!important;gap:8px!important;width:100%!important;margin:14px 0 16px!important;padding:0!important;box-sizing:border-box!important;list-style:none!important;clear:both!important}body.bj-quote-cart-page ul.btn-col2 .bj-cart-primary-action-wrap{display:block!important;flex:1 1 0!important;width:auto!important;margin:0!important;padding:0!important}body.bj-quote-cart-page ul.btn-col2 .bj-cart-primary-action-wrap+.bj-cart-primary-action-wrap{margin-left:0!important}body.bj-quote-cart-page ul.btn-col2 .bj-cart-primary-action{width:100%!important;min-height:46px!important}body.bj-quote-cart-page .bj-cart-secondary-action{margin-bottom:10px!important}}';
...
;(function billyjoJourneyAnalyticsGlobal(){
  ...
  function clickIdsFromUrl(url){
    ...
    try {
      ...
      ['fbclid','gclid','gbraid','wbraid','dclid','msclkid','ttclid','kclid','NaPm','n_media','n_query','n_rank','n_ad_group','n_ad','n_campaign_type','kmckid','kmctc'].forEach(function(k){
        var v = q.get(k);
  ...
  function platformFromClickIds(ids){
    ...
    if (ids.kmckid || ids.kmctc) return 'karrot';
    return '';
  }
  function platformFromUtm(){
    ...
    if (/^(karrot|daangn|danggeun)$/.test(source)) return 'karrot';
    return '';
  }
...
(function(){
  ...
  var DIRECT_COUPON_TITLE = '이번 달 특가 프로모션 렌탈 결합 시 추가 사은품 혜택 제공';
  function directOfferAsset(path){
    var ref = 'main';
    try {
      var src = (document.currentScript && document.currentScript.src) || '';
      var m = src.match(/billyjo-inject@([^/]+)/);
      if (m && m[1]) ref = m[1];
    } catch(_) {}
    return 'https://cdn.jsdelivr.net/gh/billyjo-appsilon/billyjo-inject@' + ref + '/' + String(path || '').replace(/^\/+/, '');
  }
  var DIRECT_HERO_BANNER_DESKTOP_URL = directOfferAsset('images/direct-offer-top-banner-desktop.jpg');
  var DIRECT_HERO_BANNER_MOBILE_URL = directOfferAsset('images/direct-offer-top-banner-mobile.jpg');
  ...
  function directCouponTitleHtml(text){
    ...
    return esc(m[1]) + '<span class="bj-do-h-sub">' + esc(m[2]) + '</span>';
  }
  ...
  function injectStyles(){
    ...
    s.textContent =
      '#bj-do-topbar{position:relative;z-index:10;background:'+BLUE+';color:#fff;font-family:Pretendard,Arial,sans-serif;font-size:12px;line-height:1.2;box-shadow:0 2px 10px rgba(0,0,0,.16)}' +
      ...
      '#bj-do-box{width:100%;max-width:760px;max-height:90vh;overflow:auto;background:#fff;border-radius:18px;box-shadow:0 24px 80px rgba(0,0,0,.32);color:#172033;display:flex;flex-direction:column;position:relative}' +
      '#bj-do-head{padding:0;width:100%;aspect-ratio:1280/269;min-height:0;background:#0838f8 url("'+DIRECT_HERO_BANNER_DESKTOP_URL+'") center/contain no-repeat;position:relative;color:#fff;overflow:hidden;flex:0 0 auto}.bj-do-badge{display:none}.bj-do-h{position:absolute!important;width:1px!important;height:1px!important;padding:0!important;margin:-1px!important;overflow:hidden!important;clip:rect(0,0,0,0)!important;white-space:nowrap!important;border:0!important}.bj-do-h-sub{display:inline-block;font-size:calc(1em - 1px);font-weight:900;margin-left:2px}.bj-do-sub{display:none}.bj-do-x{position:absolute;right:12px;top:12px;width:22px;height:22px;border:0;background:rgba(255,255,255,.94);color:'+BLUE+';font-size:15px;line-height:20px;border-radius:50%;cursor:pointer;box-shadow:0 3px 9px rgba(15,32,70,.13);padding:0}' +
      '#bj-do-body{padding:0;overflow:visible;flex:0 0 auto}.bj-do-layout{display:grid;grid-template-columns:minmax(0,1fr) 268px;gap:0;min-height:0}.bj-do-main{padding:16px 16px 18px}.bj-do-side{padding:16px;background:#f7f9ff;border-left:1px solid #e2e8f7}.bj-do-sec{margin-top:16px}.bj-do-sec:first-child{margin-top:0}.bj-do-sec h3{font-size:13px;margin:0 0 9px;font-weight:950;color:#263248}.bj-do-cats{display:flex;gap:7px;overflow:auto;padding-bottom:2px}.bj-do-chip{border:1px solid #d8e1f2;background:#fff;color:#445065;border-radius:999px;padding:8px 11px;font-size:12px;font-weight:900;white-space:nowrap}.bj-do-chip.on{border-color:'+BLUE+';background:#eef3ff;color:'+BLUE+'}.bj-do-group{display:flex;align-items:center;gap:8px;margin:12px 0 8px;font-size:12px;font-weight:950;color:#263248}.bj-do-group:after{content:"";height:1px;background:#e8edf7;flex:1}' +
      ...
      '@media(max-width:720px){#bj-do-back{align-items:flex-end;padding:0;background:rgba(10,16,30,.55)}#bj-do-box{max-width:none;max-height:92vh;border-radius:18px 18px 0 0}.bj-do-layout{display:block}.bj-do-main{padding:14px}.bj-do-side{padding:12px 14px 18px;border-left:0;border-top:1px solid #e2e8f7}#bj-do-head{aspect-ratio:750/320;background-image:url("'+DIRECT_HERO_BANNER_MOBILE_URL+'");background-position:center top!important;background-size:100% auto!important}.bj-do-x{right:12px;top:12px;width:20px;height:20px;font-size:14px;line-height:18px}.bj-do-card{grid-template-columns:58px minmax(0,1fr) 52px;gap:9px}.bj-do-card img{width:58px;height:58px}.bj-do-review{display:none}.bj-do-memo{min-height:96px}.bj-do-total-v{font-size:24px}#bj-do-jump{position:fixed;left:16px;right:16px;bottom:calc(env(safe-area-inset-bottom,0px) + 14px);z-index:100003;border:0;border-radius:13px;background:linear-gradient(180deg,#2f68ff 0%,#0838f8 54%,#0029c8 100%);color:#fff;min-height:47px;padding:11px 15px;align-items:center;justify-content:center;gap:8px;font:950 14px/1.16 Pretendard,Arial,sans-serif;letter-spacing:0;box-shadow:0 7px 0 #001d99,0 16px 27px rgba(8,56,248,.34),inset 0 1px 0 rgba(255,255,255,.42),inset 0 -1px 0 rgba(0,0,0,.13);cursor:pointer;overflow:hidden;transform:translateY(0);transition:transform .16s ease,box-shadow .16s ease;isolation:isolate}#bj-do-jump:before{content:"";position:absolute;inset:-86% -44%;background:linear-gradient(115deg,transparent 36%,rgba(255,255,255,.46) 49%,transparent 61%);transform:translateX(-74%) rotate(6deg);animation:bjDoCtaShine 3.1s ease-in-out infinite;z-index:-1}#bj-do-jump:active{transform:translateY(4px);box-shadow:0 3px 0 #001d99,0 10px 18px rgba(8,56,248,.26),inset 0 1px 0 rgba(255,255,255,.35),inset 0 -1px 0 rgba(0,0,0,.16)}#bj-do-jump:after{content:"";width:17px;height:17px;flex:0 0 auto;background:url("data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 width=%2724%27 height=%2724%27 viewBox=%270 0 24 24%27 fill=%27none%27 stroke=%27white%27 stroke-width=%273%27 stroke-linecap=%27round%27 stroke-linejoin=%27round%27%3E%3Cpath d=%27M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4%27/%3E%3Cpolyline points=%277 10 12 15 17 10%27/%3E%3Cline x1=%2712%27 y1=%2715%27 x2=%2712%27 y2=%273%27/%3E%3C/svg%3E") center/contain no-repeat}#bj-do-box.bj-do-jump-on #bj-do-jump{display:flex}}' +
  ...
  function mountFab(){
    ...
    var b = document.createElement('button'); b.id = 'bj-do-fab'; b.type = 'button';
    b.innerHTML = '<span class="bj-do-fab-ic"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 12v8a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-8h16ZM6 10a3 3 0 1 1 3-3c0 .34-.06.68-.17 1H11V5h2v3h2.17A3 3 0 1 1 18 10h2a1 1 0 0 1 1 1v1H3v-1a1 1 0 0 1 1-1h2Zm3-4a1 1 0 1 0 0 2h1.13A2.4 2.4 0 0 0 9 6Zm6 2a1 1 0 1 0 0-2 2.4 2.4 0 0 0-1.13 2H15Z"/></svg></span><span class="bj-do-fab-t">다이렉트<br>쿠폰</span><span class="bj-do-fab-x" aria-label="닫기">×</span>';
    b.onclick = function(e){
      var x = e && e.target && e.target.closest && e.target.closest('.bj-do-fab-x');
      if (x) {
        try { sessionStorage.setItem(DIRECT_FAB_CLOSED_KEY, '1'); } catch(_){}
  ...
  function displayTotal(){
    return Math.max(0, calcTotal() - 30000);
  }
  function displayTotalRange(){
    var high = displayTotal();
    var low = Math.max(0, Math.round(high * 0.85));
    return { low: low, high: high };
  }
  function weeklyCouponState(){
    var now = Date.now();
    var value = null;
    try { value = JSON.parse(localStorage.getItem(DIRECT_WEEKLY_COUPON_KEY) || 'null'); } catch(_) {}
    if (!value || !value.weeklyUntil || now > Number(value.weeklyUntil)) {
      value = { startedAt: now, expiresAt: now + 30 * 60000, weeklyUntil: now + 7 * 24 * 60 * 60000 };
      try { localStorage.setItem(DIRECT_WEEKLY_COUPON_KEY, JSON.stringify(value)); } catch(_) {}
    }
    return {
      active: now <= Number(value.expiresAt || 0),
      expiresAt: Number(value.expiresAt || 0),
      weeklyUntil: Number(value.weeklyUntil || 0)
    };
  }
  function couponCountdownText(coupon){
    var left = Math.max(0, (coupon && coupon.expiresAt ? coupon.expiresAt : 0) - Date.now());
    var m = Math.floor(left / 60000);
    var s = Math.floor((left % 60000) / 1000);
    return pad(m) + ':' + pad(s);
  }
  function renderDisplayTotalHtml(){
    var range = displayTotalRange();
    var coupon = weeklyCouponState();
    var couponHtml = coupon.active
      ? '<div class="bj-do-coupon-wrap"><div class="bj-do-coupon-card" role="img" aria-label="1주일에 한번 오늘만 30,000포인트 지원금 쿠폰"><i aria-hidden="true"></i><div class="bj-do-coupon-copy"><span>1주일에 한번! 오늘만</span><em>지원금 쿠폰</em></div><b class="bj-do-coupon-amount">30,000p</b></div><span class="bj-do-coupon-time">쿠폰 유효시간 <b data-bj-do-coupon-left>' + esc(couponCountdownText(coupon)) + '</b> · 7일 1회 한정</span></div>'
      : '';
    return '<span class="bj-do-total-range" data-low="' + range.low + '" data-high="' + range.high + '">' +
      '<span class="bj-do-num bj-do-num-low">' + esc(won(range.low)) + '</span><span class="bj-do-range-sep">~</span><span class="bj-do-num bj-do-num-high">' + esc(won(range.high)) + '</span></span>' + couponHtml;
  }
  function totalRangeText(){
    return '';
  }
  function animateTotalNumbers(box){
    var wrap = box && box.querySelector && box.querySelector('.bj-do-total-range');
    if (!wrap) return;
    var lowEl = wrap.querySelector('.bj-do-num-low');
    var highEl = wrap.querySelector('.bj-do-num-high');
    var low = parseInt(wrap.getAttribute('data-low') || '0', 10) || 0;
    var high = parseInt(wrap.getAttribute('data-high') || '0', 10) || 0;
    var key = low + ':' + high;
    if (wrap.getAttribute('data-animated-key') === key) return;
    wrap.setAttribute('data-animated-key', key);
    var v = box.querySelector('.bj-do-total-v');
    if (v) v.classList.add('is-calculating');
    var fromLow = Math.max(0, Math.round(low * 0.82));
    var fromHigh = Math.max(0, Math.round(high * 0.82));
    var start = Date.now();
    var dur = 850;
    function tick(){
      var t = Math.min(1, (Date.now() - start) / dur);
      var eased = 1 - Math.pow(1 - t, 3);
      if (lowEl) lowEl.textContent = won(fromLow + (low - fromLow) * eased);
      if (highEl) highEl.textContent = won(fromHigh + (high - fromHigh) * eased);
      if (t < 1) requestAnimationFrame(tick);
      else if (v) v.classList.remove('is-calculating');
    }
    tick();
  }
  function startCouponCountdown(box){
    var coupon = weeklyCouponState();
    if (state.couponTimer) clearInterval(state.couponTimer);
    function render(){
      var el = box && box.querySelector && box.querySelector('[data-bj-do-coupon-left]');
      if (!el) return;
      el.textContent = couponCountdownText(coupon);
      if (Date.now() > coupon.expiresAt) {
        clearInterval(state.couponTimer);
        state.couponTimer = null;
        if (document.body.contains(box)) refreshModal(box);
      }
    }
    render();
    if (coupon.active) state.couponTimer = setInterval(render, 1000);
  }
  function makeMemo(){
    ...
    var lines = [
      ...
      'AI 예상 지원금 합계: ' + won(displayTotalRange().low) + ' ~ ' + won(displayTotalRange().high) + ' + 30,000원 지원금 쿠폰'
    ];
  ...
  function saveDirectQuoteContext(){
    try {
      ...
      var ctx = {
        ...
        memo: memo,
        expectedGiftAmount: displayTotal(),
        originalExpectedGiftAmount: calcTotal(),
  ...
  function quoteDelay(){
    return new Promise(function(resolve){
      setTimeout(resolve, 3000 + Math.floor(Math.random() * 2001));
    });
  }
  function showQuoteLoading(box){
    if (box.querySelector('.bj-do-loading')) return;
    var div = document.createElement('div');
  ...
  function refreshModal(box){
    ...
    box.querySelector('.bj-do-total-v').innerHTML = renderDisplayTotalHtml();
    ...
    if (copyBtn) {
      copyBtn.textContent = '지원금 쿠폰 받고 신청';
    }
    ...
    syncQuoteJumpButton(box);
  }
  function syncQuoteJumpButton(box){
    var jump = box && box.querySelector && box.querySelector('#bj-do-jump');
    var scroller = getDirectOfferScroller(box);
    var cta = box && box.querySelector && box.querySelector('.bj-do-copy');
    if (!jump || !scroller || !cta) return;
    if (!window.matchMedia || !window.matchMedia('(max-width:720px)').matches || cta.disabled || box.querySelector('.bj-do-loading')) {
      box.classList.remove('bj-do-jump-on');
      return;
    }
    var sr = scroller.getBoundingClientRect();
    var cr = cta.getBoundingClientRect();
    var ctaVisible = cr.top < (sr.bottom - 18) && cr.bottom > (sr.top + 18);
    var canScroll = (scroller.scrollHeight - scroller.scrollTop - scroller.clientHeight) > 22;
    box.classList.toggle('bj-do-jump-on', !ctaVisible && canScroll);
  }
  function getDirectOfferScroller(box){
    if (!box) return null;
    if (window.matchMedia && window.matchMedia('(max-width:720px)').matches) return box;
    return (box.querySelector && box.querySelector('#bj-do-body')) || box;
  }
  function mountQuoteJumpBehavior(box){
    var jump = box && box.querySelector && box.querySelector('#bj-do-jump');
    var scroller = getDirectOfferScroller(box);
    var cta = box && box.querySelector && box.querySelector('.bj-do-copy');
    if (!jump || !scroller || !cta) return;
    function onScroll(){ syncQuoteJumpButton(box); }
    jump.onclick = function(){
      directOfferTrack('direct_offer_scroll_to_submit', {
        expected_gift_low: displayTotalRange().low,
        expected_gift_high: displayTotalRange().high
      });
      box.classList.remove('bj-do-jump-on');
      try { cta.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
      catch(_) { scroller.scrollTop = scroller.scrollHeight; }
      requestAnimationFrame(function(){ syncQuoteJumpButton(box); });
      setTimeout(onScroll, 120);
      setTimeout(onScroll, 450);
    };
    scroller.addEventListener('scroll', onScroll, { passive: true });
    window.addEventListener('resize', onScroll);
    box.__bjDoJumpCleanup = function(){
  ...
  function openModal(proceed, opts){
      ...
      '<div id="bj-do-body">' +
        '<div class="bj-do-layout">' +
          ...
          '<div class="bj-do-side">' +
            '<div id="bj-do-total"><div class="bj-do-total-k">AI 예상 지원금 합계</div><div class="bj-do-total-formula">예상 사은품 + 지원금 쿠폰</div><div class="bj-do-total-v is-calculating">계산 중입니다</div><div class="bj-do-total-sub">지원금 합계금을 계산 중입니다.</div></div>' +
            '<textarea class="bj-do-memo" readonly></textarea>' +
            '<button type="button" class="bj-do-copy">지원금 쿠폰 받고 신청</button>' +
            '<div class="bj-do-note">선택 상품은 견적비교함에 담긴 뒤 24시간 견적 인증/신청 단계로 이동합니다.</div>' +
          '</div>' +
        '</div>' +
      '</div>' +
      '<button type="button" id="bj-do-jump">지원금 쿠폰 받고 신청</button>';
    ...
    function close(){
      state.opened = false;
      if (state.couponTimer) { clearInterval(state.couponTimer); state.couponTimer = null; }
      if (box.__bjDoJumpCleanup) { try { box.__bjDoJumpCleanup(); } catch(_){} }
    ...
    box.querySelector('.bj-do-copy').onclick = function(){
      ...
      setTimeout(function(){ var b = box.querySelector('.bj-do-copy'); if (b) b.textContent = '지원금 쿠폰 받고 신청'; }, 1400);
    };
    mountQuoteJumpBehavior(box);
  ...
  (function injectHomeCategoryQuickLinks() {
    ...
    function addStyle() {
      ...
      style.textContent = [
        ...
        '#bj-home-cat-links .bj-hcat-more-wrap{display:flex;justify-content:center;margin:12px 0 0}',
        '#bj-home-cat-links .bj-hcat-more-btn{display:inline-flex;align-items:center;justify-content:center;width:min(420px,100%);height:36px;padding:0 18px;border:1px solid #d8e7f7;border-radius:8px;background:var(--bj-home-cat-bg)!important;color:#0838f8;font-size:13px;font-weight:700;line-height:1;letter-spacing:0;cursor:pointer;box-shadow:0 4px 10px rgba(35,80,130,.05)}',
        '#bj-home-cat-links .bj-hcat-more-btn:hover{background:var(--bj-home-cat-bg)!important;border-color:#b9d2f5;color:#062fd6}',
...
function bjHeaderMainInit() {
  ...
  if (true) {  // 전체 페이지 적용 (2026-07-02): 이전엔 /prod_list/ URL만 → 메인페이지 등 .prod_list .item 카드에도 적용. bjpRun은 .prod_list .item만 대상이라 카드 없는 페이지는 no-op.
    ...
    var PRELOADED_PRICE_SNAPSHOT_DATE = '2026-09-23';
    var PRELOADED_PRICE_SNAPSHOT = {"ok":true,"by_model":{"CHP-1111N":{"reward":17810,"name":"엘리트 빌트인"},"CHP-1111":{"reward":17810,"name":"엘리트 빌트인"},"CHP-1112N":{"reward":17810,"name":"엘리트 빌트인"},"CHP-1112":{"reward":17810,"name":"엘리트 빌트인"},"CHP-5610N 재고 소진시 즉시 단종":{"reward":13300,"name":"아이콘 스탠드"},"CHP-5610N재고소진시즉시단종":{"reward":13300,"name":"아이콘 스탠드"},"CHP-5711L":{"reward":13310,"name":"워터스탠드2"},"CHP-5711":{"reward":13310,"name":"워터스탠드2"},"CHP-5711R":{"reward":15110,"name":"워터스탠드2"},"CHP-5730L":{"reward":13310,"name":"워터스탠드플러스"},"CHP-5730":{"reward":13310,"name":"워터스탠드플러스"},"CHP-5730R":{"reward":15110,"name":"워터스탠드플러스"},"CHP-5730N":{"reward":15110,"name":"워터스탠드플러스N"},"CHP-6340L":{"reward":17810,"name":"엘리트 정수기"},"CHP-6340":{"reward":17810,"name":"엘리트 정수기"},"CP-6340L":{"reward":16010,"name":"엘리트 정수기"},"CP-6340":{"reward":16010,"name":"엘리트 정수기"},"CHP-7211N":{"reward":15560,"name":"아이콘2.0"},"CHP-7211":{"reward":15560,"name":"아이콘2.0"},"CP-7211N":{"reward":13760,"name":"아이콘2.0"},"CP-7211":{"reward":13760,"name":"아이콘2.0"},"CHP-7220N":{"reward":16010,"name":"아이콘3.0"},"CHP-7220":{"reward":16010,"name":"아이콘3.0"},"CP-7220N":{"reward":14210,"name":"아이콘3.0"},"CP-7220":{"reward":14210,"name":"아이콘3.0"},"CHP-7212N":{"reward":16910,"name":"아이콘 프로"},"CHP-7212":{"reward":16910,"name":"아이콘 프로"},"CHPI-5820L":{"reward":38060,"name":"아이스 스탠드3.0 정수기"},"CHPI-5820":{"reward":38060,"name":"아이스 스탠드3.0 정수기"},"CHPI-5801LB":{"reward":41210,"name":"아이스 스탠드"},"CHPI-5801LW":{"reward":40310,"name":"아이스 스탠드"},"CHPI-620L":{"reward":34010,"name":"아이스 스탠드 슬림"},"CHPI-620":{"reward":34010,"name":"아이스 스탠드 슬림"},"CHPI-7430N":{"reward":28610,"name":"아이콘얼음 미니"},"CHPI-7430":{"reward":28610,"name":"아이콘얼음 미니"},"CHPI-7410N":{"reward":31310,"name":"아이콘얼음 스탠다드"},"CHPI-7410":{"reward":31310,"name":"아이콘얼음 스탠다드"},"CPI-7410N":{"reward":27710,"name":"아이콘얼음 스탠다드"},"CPI-7410":{"reward":27710,"name":"아이콘얼음 스탠다드"},"CHPI-7420N":{"reward":34010,"name":"아이콘얼음 맥스"},"CHPI-7420":{"reward":34010,"name":"아이콘얼음 맥스"},"CHPI-7400N":{"reward":30860,"name":"아이콘얼음정수기"},"CHPI-7400":{"reward":30860,"name":"아이콘얼음정수기"},"CPI-7400N":{"reward":27260,"name":"아이콘얼음정수기"},"CPI-7400":{"reward":27260,"name":"아이콘얼음정수기"},"CHPI-7511L":{"reward":28160,"name":"AIS 3.0"},"CHPI-7511":{"reward":28160,"name":"AIS 3.0"},"CPI-7511L":{"reward":24560,"name":"AIS 3.0"},"CPI-7511":{"reward":24560,"name":"AIS 3.0"},"CHPI-7521L":{"reward":26360,"name":"얼음정수기 RO"},"CHPI-7521":{"reward":26360,"name":"얼음정수기 RO"},"CPSI-8510L":{"reward":34010,"name":"AIS 3.0 스파클링"},"CPSI-8510":{"reward":34010,"name":"AIS 3.0 스파클링"},"AP-1023F":{"reward":6560,"name":"싱글파워"},"AP-1023":{"reward":6560,"name":"싱글파워"},"AP-1523D":{"reward":15110,"name":"멀티액션"},"AP-1523":{"reward":15110,"name":"멀티액션"},"AP-1623M":{"reward":17810,"name":"노블청정기"},"AP-1623":{"reward":17810,"name":"노블청정기"},"AP-2023K":{"reward":20510,"name":"노블청정기"},"AP-2023":{"reward":20510,"name":"노블청정기"},"AP-3021D":{"reward":33110,"name":"노블청정기"},"AP-3021":{"reward":33110,"name":"노블청정기"},"AP-3024H":{"reward":33110,"name":"노블청정기"},"AP-3024":{"reward":33110,"name":"노블청정기"},"AP-4025D":{"reward":36710,"name":"노블청정기"},"AP-4025":{"reward":36710,"name":"노블청정기"},"APD-1023A":{"reward":20510,"name":"듀얼클린 제습"},"APD-1023":{"reward":20510,"name":"듀얼클린 제습"},"APD-1025E":{"reward":24110,"name":"노블 제습공기청정기"},"APD-1025":{"reward":24110,"name":"노블 제습공기청정기"},"BA36-B":{"reward":6560,"name":"프라임 비데"},"BAS37-C":{"reward":11960,"name":"더블케어"},"BAS38-C":{"reward":12860,"name":"더블케어플러스"},"BAS40-A":{"reward":6560,"name":"스스로케어(일반)"},"BAS41-A":{"reward":7910,"name":"스타일케어"},"BAS49-A":{"reward":9710,"name":"슬리믹"},"MB-M05M":{"reward":69900,"name":"리클라이닝 마사지셋"},"MB-M05":{"reward":69900,"name":"리클라이닝 마사지셋"},"MB-M03M":{"reward":63100,"name":"마사지셋"},"MB-M03":{"reward":63100,"name":"마사지셋"},"MB-R05M":{"reward":63950,"name":"리클라이닝 코어셋"},"MB-R05":{"reward":63950,"name":"리클라이닝 코어셋"},"MB-R03M":{"reward":57150,"name":"코어셋"},"MB-R03":{"reward":57150,"name":"코어셋"},"MC-B02":{"reward":30410,"name":"마인플러스"},"MC-B03":{"reward":28610,"name":"마인2"},"MC-C01":{"reward":41900,"name":"페블체어"},"MC-C02AT":{"reward":46900,"name":"페블체어2"},"MC-R01":{"reward":47900,"name":"트리플체어"},"MC-R01M":{"reward":49900,"name":"트리플체어(바퀴형)"},"WI-37C90720N":{"reward":26900,"name":"WI-37C90720N"},"WI-37C90720":{"reward":26900,"name":"WI-37C90720N"},"WI-36C90620N":{"reward":29900,"name":"WI-36C90620N"},"WI-36C90620":{"reward":29900,"name":"WI-36C90620N"},"WI-36C80620N":{"reward":29900,"name":"WI-36C80620N"},"WI-36C80620":{"reward":29900,"name":"WI-36C80620N"},"WI-60C9600M":{"reward":29900,"name":"WI-60C9600M"},"WI-60C9600":{"reward":29900,"name":"WI-60C9600M"},"WI-55S90510M":{"reward":29900,"name":"WI-55S90510M"},"WI-55S90510":{"reward":29900,"name":"WI-55S90510M"},"WI-55S9P510M":{"reward":30900,"name":"WI-55S9P510M"},"WI-55S9P510":{"reward":30900,"name":"WI-55S9P510M"},"WI-70S90010M":{"reward":33900,"name":"WI-70S90010M"},"WI-70S90010":{"reward":33900,"name":"WI-70S90010M"},"WI-70S9P010M":{"reward":34900,"name":"WI-70S9P010M"},"WI-70S9P010":{"reward":34900,"name":"WI-70S9P010M"},"WI-15C7500":{"reward":63900,"name":"WI-15C7500"},"WI-60C90511M":{"reward":29900,"name":"WI-60C90511M"},"WI-60C90511":{"reward":29900,"name":"WI-60C90511M"},"WI-60C80511M":{"reward":27900,"name":"WI-60C80511M"},"WI-60C80511":{"reward":27900,"name":"WI-60C80511M"},"WI-80S9P510M":{"reward":37900,"name":"WI-80S9P510M"},"WI-80S9P510":{"reward":37900,"name":"WI-80S9P510M"},"WP-30B90620N":{"reward":23900,"name":"WP-30B90620N"},"WP-30B90620":{"reward":23900,"name":"WP-30B90620N"},"WP-60C90010M":{"reward":18900,"name":"WP-60C90010M"},"WP-60C90010":{"reward":18900,"name":"WP-60C90010M"},"WP-60C90511M":{"reward":17900,"name":"WP-60C90511M"},"WP-60C90511":{"reward":17900,"name":"WP-60C90511M"},"WP-45C90020M":{"reward":11900,"name":"WP-45C90020M"},"WP-45C90020":{"reward":11900,"name":"WP-45C90020M"},"WP-35C90010N":{"reward":11900,"name":"WP-35C90010N"},"WP-35C90010":{"reward":11900,"name":"WP-35C90010N"},"WP-46S90510M":{"reward":18900,"name":"WP-46S90510M"},"WP-46S90510":{"reward":18900,"name":"WP-46S90510M"},"WP-46S9P510M":{"reward":20900,"name":"WP-46S9P510M"},"WP-46S9P510":{"reward":20900,"name":"WP-46S9P510M"},"WP-50S90010M":{"reward":13900,"name":"WP-50S90010M"},"WP-50S90010":{"reward":12900,"name":"WP-50S90010U"},"WP-50S90010U":{"reward":12900,"name":"WP-50S90010U"},"WP-31S90010N":{"reward":20900,"name":"WP-31S90010N"},"WP-31S90010":{"reward":20900,"name":"WP-31S90010N"},"WP-45S9P010M":{"reward":22900,"name":"WP-45S9P010M"},"WP-45S9P010":{"reward":22900,"name":"WP-45S9P010M"},"WP-80S9P510M":{"reward":29900,"name":"WP-80S9P510M"},"WP-80S9P510":{"reward":29900,"name":"WP-80S9P510M"},"WP-60C80511M":{"reward":15900,"name":"WP-60C80511M"},"WP-60C80511":{"reward":15900,"name":"WP-60C80511M"},"WP-43C60000M":{"reward":9900,"name":"WP-43C60000M"},"WP-43C60000":{"reward":9900,"name":"WP-43C60000M"},"WP-40C60000M":{"reward":5900,"name":"WP-40C60000M"},"WP-40C60000":{"reward":5900,"name":"WP-40C60000M"},"WP-50C90621N":{"reward":19900,"name":"WP-50C90621N"},"WP-50C90621":{"reward":19900,"name":"WP-50C90621N"},"WP-50C80621N":{"reward":16900,"name":"WP-50C80621N"},"WP-50C80621":{"reward":16900,"name":"WP-50C80621N"},"WP-30C8460N":{"reward":37900,"name":"WP-30C8460N"},"WP-30C8460":{"reward":37900,"name":"WP-30C8460N"},"WP-46S90510N":{"reward":20900,"name":"WP-46S90510N"},"WU923A":{"reward":22900,"name":"듀얼"},"WU923":{"reward":22900,"name":"듀얼"},"CP-AMS100":{"reward":14210,"name":"직수- 데스크형"},"CP-ABS100":{"reward":12410,"name":"직수- 데스크형"},"CP-ABSC100":{"reward":13310,"name":"직수- 데스크형"},"CP-AHS100":{"reward":25010,"name":"얼음- 데스크형"},"CP-AHSC100":{"reward":25910,"name":"얼음- 데스크형"},"CP-AJS801SW":{"reward":28160,"name":"얼음 -스탠드형"},"CP-AJS821SW":{"reward":28160,"name":"얼음 -스탠드형"},"CBT-KFFS31RW":{"reward":2660,"name":"비데"},"CBT-QSB1041W":{"reward":6060,"name":"비데"},"CBT-QSB1041":{"reward":6060,"name":"비데"},"CBT-PFF31RW":{"reward":1810,"name":"비데"},"CBT-QSF1041W":{"reward":3510,"name":"비데"},"CBT-QSF1041":{"reward":3510,"name":"비데"}}}; // scripts/update-price-snapshot.mjs가 매일 최신값으로 채운다.
    ...
    if (isVehicleOrder) {
      ...
      if (memoArea) {
        memoArea.placeholder =
          '[사은품 지급 안내]\n' +
          "이전 단계에서 안내받으신 '사은품 코드'가 자동으로 입력되어 있어요.\n\n" +
          '[필수 확인 및 기재 항목]\n' +
          '1. 설치 희망일, 희망 색상, 기타 요청사항을 자유롭게 적어주세요.\n' +
          '2. 사은품 수령 및 렌탈료 결제에 필요한 고객님 명의의 계좌정보를 적어주세요.\n' +
          "3. 제휴카드나 카드결제를 원하시면 '카드결제'와 카드명을 함께 적어주세요. 담당자가 접수 방법을 안내드릴게요.\n" +
          '(카드 결제를 원하셔도 접수 신청을 위해 2번 계좌정보는 함께 필요합니다.)';
      }
    }
  ...
  function reorderHome(pageEl) {
    ...
    if (!document.getElementById('bj-refine-css')) {
      ...
      bjRf.textContent = '#bj-v5-injected .pill-wrap{display:none !important}' +
        ...
        // PC 홈 하단 콘텐츠 폭을 히어로/상단 카테고리와 같은 1280px 기준으로 통일 (2026-08-12)
        "@media(min-width:1280px){#bj-v5-injected{width:1280px !important;max-width:1280px !important;margin-left:auto !important;margin-right:auto !important;padding-left:0 !important;padding-right:0 !important}#bj-v5-injected>.hero,#bj-v5-injected>div:not(.zone){width:1280px !important;max-width:1280px !important;margin-left:auto !important;margin-right:auto !important}#bj-v5-injected .zone{width:100vw !important;margin-left:calc((1280px - 100vw)/2) !important;margin-right:calc((1280px - 100vw)/2) !important;padding-left:0 !important;padding-right:0 !important}#bj-v5-injected .zone>section{width:1280px !important;max-width:1280px !important;margin-left:auto !important;margin-right:auto !important}}" +
    ...
    if (!document.getElementById('bj-trust-partners-css')) {
      ...
      bjTrustCss.textContent =
        ...
        '.bj-trust-partners{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);align-items:center;justify-content:center;gap:10px;margin:16px auto 0;width:min(100%,330px);padding:0 10px;box-sizing:border-box}' +
        '.bj-trust-partner{width:100%;min-width:0;height:54px;padding:10px 14px;border-radius:12px;background:#fff;border:1px solid rgba(8,56,248,.12);box-shadow:0 10px 24px rgba(8,28,92,.1);display:flex;align-items:center;justify-content:center;box-sizing:border-box}' +
        ...
        '@media(max-width:420px){.hero[data-bj-trust="1"] h1{font-size:24px!important;line-height:1.24!important}.bj-trust-partners{gap:8px;margin-top:14px;width:100%;padding:0 10px}.bj-trust-partner{height:50px;padding:9px 10px}.bj-trust-actions{width:calc(100% - 24px)}.bj-trust-cta{font-size:14px;min-height:46px;padding:13px 14px}}';
...
";
  ...
  var RECO_REWARD_BY_MODEL = {
    'CHPI-620L': 34010, 'CHPI-620': 34010,
    'WI-36C90620N': 29900, 'WI-36C90620': 29900,
    'CHPI-7400N': 30860, 'CHPI-7400': 30860,
    'CHP-6340L': 17810, 'CHP-6340': 17810,
    'WU923A': 22900, 'WU923': 22900,
    'CHP-7212N': 16910, 'CHP-7212': 16910,
    'AP-3021D': 33110, 'AP-3021': 33110,
    'CBT-QSB1041W': 6060, 'CBT-QSB1041': 6060
  };

  function rewardForRecommendation(item) {
    var text = (((item && item.model) || '') + ' ' + ((item && item.name) || '')).toUpperCase();
    var keys = Object.keys(RECO_REWARD_BY_MODEL).sort(function(a, b) { return b.length - a.length; });
    for (var i = 0; i < keys.length; i++) {
  ...
  function renderCard(item, idx) {
    ...
    var reward = rewardForRecommendation(item);
...
(function(){
  ...
  var CSS = [
    ...
    '}',
    'body:has(.prod_view_bot.card.mt40) .new-qb{',
      '  right:14px !important; z-index:99997 !important;',
    '}',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick{ background:transparent !important; box-shadow:none !important }',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick .link{',
      '  display:flex !important; flex-direction:column !important; align-items:center !important; gap:7px !important;',
      '  padding:7px 5px !important; background:#fff !important; border:1px solid #e1e7f2 !important;',
      '  border-radius:18px !important; box-shadow:0 8px 24px rgba(15,32,70,.14) !important;',
    '}',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick .link>p{',
      '  display:block !important; position:relative !important; width:42px !important; height:42px !important;',
      '  margin:0 !important; padding:0 !important; float:none !important; clear:none !important;',
    '}',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick .link>p.show-767,',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick_list,',
    'body:has(.prod_view_bot.card.mt40) .new-qb .goTop{ display:none !important }',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick .link>p>a,',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick .link .bj-ai-quote-quick a{',
      '  width:42px !important; height:42px !important; border-radius:999px !important; box-sizing:border-box !important;',
      '  display:flex !important; align-items:center !important; justify-content:center !important;',
      '  background:#f7f9ff !important; border:1px solid #dfe7fb !important; color:#0838f8 !important;',
      '  box-shadow:none !important; padding:0 !important;',
    '}',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick .link>p>a:hover,',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick .link .bj-ai-quote-quick a:hover{ background:#eef3ff !important }',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick .link>p>a>img,',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick .link .bj-ai-quote-quick img.bj-ai-quote-icon{',
      ...
      '  display:block !important; object-fit:contain !important; margin:0 !important; pointer-events:none !important;',
    '}',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick .link>p>a>span{ display:none !important }',
    'body:has(.prod_view_bot.card.mt40) .new-qb .quick .link .bj-ai-quote-quick .bj-ai-quote-label{ display:none !important }',
    ...
    '@media (max-width:600px){',
      ...
      '  body:has(.prod_view_bot.card.mt40) .new-qb{ right:10px !important }',
      '  body:has(.prod_view_bot.card.mt40) .new-qb .quick .link{ gap:6px !important; padding:6px 4px !important }',
      '  body:has(.prod_view_bot.card.mt40) .new-qb .quick .link>p{ width:40px !important; height:40px !important }',
      '  body:has(.prod_view_bot.card.mt40) .new-qb .quick .link>p>a,',
      '  body:has(.prod_view_bot.card.mt40) .new-qb .quick .link .bj-ai-quote-quick a{ width:40px !important; height:40px !important }',
    ...
    '.new-qb .quick .link #bj-fs-ctrl{ display:flex; justify-content:center; margin:0 }',
    '#bj-fs-ctrl .bj-fs-inner{ display:flex; flex-direction:column; align-items:center; gap:4px; background:#fff; border:1px solid #e6e8ee; border-radius:14px; padding:5px 4px; box-shadow:none }',
    '#bj-fs-ctrl .bj-fs-lab{ font-size:10px; font-weight:700; color:#8a909a; line-height:1; letter-spacing:-0.3px }',
    '#bj-fs-ctrl .bj-fs-btn{ display:inline-flex; align-items:center; justify-content:center; width:31px; height:31px; padding:0; border:0; background:#f2f5ff; color:#0838f8; cursor:pointer; border-radius:50%; line-height:1; transition:background .12s, transform .1s }',
    '#bj-fs-ctrl .bj-fs-btn svg{ width:19px; height:19px; display:block }',
  ...
  function bjReadUtm(){
    try {
      var q = new URLSearchParams(location.search), o = {}, ks = ['source','medium','campaign','content','term','id','source_platform'];
      for (var i = 0; i < ks.length; i++){ var v = q.get('utm_' + ks[i]); if (v) o[ks[i]] = v; }
      ['ch','kmckid','kmctc'].forEach(function(k){ var v = q.get(k); if (v) o[k] = v; });
  ...
  function bjPlatformFromUtm(utm){
    ...
    if (/^(karrot|daangn|danggeun)$/.test(s)) return 'karrot';
    return '';
  }
  ...
  function bjReadAttribution(){
    ...
    if (platform === 'karrot') {
      if (utm.id) attr.campaignId = utm.id;
      if (utm.term) attr.adsetId = utm.term;
      if (utm.content) {
        attr.adId = utm.content;
        attr.creativeId = utm.content;
      }
    }
    if (fbp) attr.fbp = fbp;
    if (fbc) attr.fbc = fbc;
  ...
  function bjAnalyticsPayload(eventName, extra){
    ...
    var payload = {
      ...
      conversion_platform: attr.adPlatform || '',
      ad_platform: attr.adPlatform || '',
      kmckid: attr.kmckid || (attr.clickIds && attr.clickIds.kmckid) || '',
      kmctc: attr.kmctc || (attr.clickIds && attr.clickIds.kmctc) || '',
      karrot_campaign_id: attr.campaignId || '',
      karrot_ad_group_id: attr.adsetId || '',
      karrot_material_id: attr.adId || attr.creativeId || '',
      attribution_model: 'last_click_only',
  ...
  function _trackCatalogViewContent(){
    var isProdView = /\/prod_view\/\d+/.test(location.pathname || '');
    if (!isProdView || window.__bjMetaCatalogViewTracked) return;
    var prodNo = (location.pathname.match(/prod_view\/(\d+)/) || [])[1] || '';
    var id = _metaCatalogId(prodNo);
    if (!id) return;
  ...
  function buildWidgetSelector(wrapper, handle){
    ...
    function syncTopChips(term){
      ...
      /* 타사보상 약정(term.el === null)도 상단에 그대로 반영한다 — 기준은 위젯이다.
         ⚠️ 이 약정은 native 약정행이 없어 주문폼에 실리지 않는다(상단 43,560 vs 접수 51,900).
            주문 연결은 별건으로 남아 있다 — pill 제거/비활성 결정 전까지 유효한 제약. */
      var priceLabel = wonLabel(term.price, term.priceNum);
  ...
  function fetchAndInjectReviews(){
    ...
    if(!anchor){
      ...
      var benefitRoot = document.getElementById('bj-detail-benefit-bundle');
      parent=topEl.parentNode; anchor=(benefitRoot && benefitRoot.parentNode === parent) ? benefitRoot.nextSibling : topEl.nextSibling; // 폴백: 이미지+가격+혜택 다음
    }
 ...
 *   카운트는 새벽 cron이 inject.js에 미리 박아둔 스냅샷만 사용한다.
 *   제품 페이지 로드 중 후기 API를 호출하지 않아 첫 렌더링 지연을 만들지 않는다.
...
(function(){
  ...
  var PRELOADED_COUNTS_DATE = '2026-09-23';
  var PRELOADED_COUNTS = {"ok":true,"by_model":{"ESF5512LOX":{"n":11,"avg":4.1},"CHP-7220N":{"n":438,"avg":4.9},"CHPI-7430N":{"n":427,"avg":4.9},"CVC-G820NWM":{"n":215,"avg":4.8},"WI-37C90720N":{"n":173,"avg":4.8},"SC-D0210":{"n":100,"avg":4.9},"CMC-A17":{"n":100,"avg":4.8},"WQ652NWR":{"n":78,"avg":4.9},"DXTE120-MPK":{"n":100,"avg":4.9},"WS674MWA":{"n":273,"avg":4.9},"ATTM115-MSK":{"n":119,"avg":4.9},"CHPI-7400N":{"n":74,"avg":4.9},"WP-30C8460N":{"n":145,"avg":4.9},"WPU-IAC425":{"n":137,"avg":4.9},"CHP-7211N":{"n":100,"avg":4.9},"AT5M200-MWK":{"n":201,"avg":4.9},"AARB-0101GAWSD":{"n":59,"avg":4.8},"EWNE067-PWK":{"n":201,"avg":4.7},"AP107CGA":{"n":54,"avg":4.9},"WR872MWA":{"n":132,"avg":4.9},"AARB-0061GAWSD":{"n":37,"avg":4.8},"CDHC-140AYLWWYH":{"n":73,"avg":4.7},"DH-V06201EB":{"n":222,"avg":4.7},"CIR-ENL101FW":{"n":200,"avg":4.8},"AT8E430-MWK":{"n":212,"avg":4.9},"CIM-AS09M10S":{"n":216,"avg":4.8},"CMC-A200":{"n":100,"avg":4.8},"HS4E400-OYK":{"n":113,"avg":4.8},"WS672MWA":{"n":258,"avg":4.9},"HW300BBB":{"n":35,"avg":4.9},"CHP-6201N":{"n":224,"avg":4.9},"CP-6201N":{"n":221,"avg":4.9},"WQ672NWR":{"n":310,"avg":4.9},"CFDI-E201DCS":{"n":198,"avg":4.3},"WPU-JAC125":{"n":137,"avg":4.9},"WI-53C9600M":{"n":225,"avg":4.9},"BP750CWA":{"n":32,"avg":5},"AR626CBA":{"n":40,"avg":4.7},"CHP-320N":{"n":120,"avg":4.9},"WP-53C90020M":{"n":160,"avg":4.8},"WPU-IAC606":{"n":134,"avg":4.9},"WPU-JAC115":{"n":225,"avg":4.9},"DH-T12701GW":{"n":238,"avg":4.9},"AZSE430-JWK":{"n":100,"avg":4.8},"APRM833-JWK":{"n":100,"avg":4.9},"CFD-FNL201DCGW":{"n":223,"avg":4.8},"EWIE067-PTK":{"n":181,"avg":4.7},"F9WT":{"n":18,"avg":4.9},"WPUJAC104":{"n":111,"avg":4.9},"CMC-A350":{"n":99,"avg":4.9},"WP610NWA":{"n":193,"avg":4.9},"CMC-A230":{"n":100,"avg":4.7},"WPU-JAC104":{"n":310,"avg":4.9},"CA-BWNLDE0630NW":{"n":102,"avg":4.5},"DXJE193-LMK":{"n":100,"avg":4.8},"BD-35D51":{"n":229,"avg":4.9},"WP-50C90621N":{"n":149,"avg":4.8},"AC-28AHNL20FNW":{"n":117,"avg":4.8},"DXAE120-NEK":{"n":231,"avg":4.7},"BP550CWA":{"n":17,"avg":4.9},"BR550CWA":{"n":13,"avg":5},"BD-36D50":{"n":1,"avg":5},"BQ450RWA":{"n":15,"avg":4.8},"WQ171UWA":{"n":15,"avg":5},"AS526CWA":{"n":14,"avg":5},"BAS40-A":{"n":100,"avg":4.9},"CHP-7212N":{"n":224,"avg":4.9},"SC5GMR80A":{"n":56,"avg":5},"AT8E430-MSK":{"n":101,"avg":4.7},"WM071RWA":{"n":4,"avg":5},"WPU-IAC506":{"n":45,"avg":4.9},"DXAW120-NYK":{"n":100,"avg":4.7},"AX920BWE":{"n":48,"avg":4.9},"CBT-G1031W(셀프)":{"n":137,"avg":4.8},"DN4M180-NYK":{"n":66,"avg":4.8},"DH-YNL1652FEB":{"n":236,"avg":4.8},"BA34-B":{"n":107,"avg":4.7},"HS4E400-OBK":{"n":100,"avg":4.7},"CP-7220N":{"n":44,"avg":4.8},"CDHC-180AYMWWYH":{"n":4,"avg":5},"BN150":{"n":107,"avg":4.9},"CHPI-7410N":{"n":194,"avg":4.9},"MSDNG30-OWK":{"n":133,"avg":4.5},"ATTM115-MWK":{"n":118,"avg":4.9},"MHCNB00-OWK":{"n":28,"avg":4.8},"DN3E170-LWK":{"n":119,"avg":4.8},"ACL-15C1A":{"n":3,"avg":5},"WPU-JAC115P":{"n":38,"avg":4.8},"DXWE228-OYK":{"n":241,"avg":4.9},"WPU-IAC425P":{"n":24,"avg":4.9},"AR318CWA":{"n":14,"avg":4.9},"DCVE160-PEK":{"n":117,"avg":4.9},"AD-2026C":{"n":4,"avg":5},"CPI-7410N":{"n":10,"avg":5},"CP-320N":{"n":118,"avg":4.8},"WPU-IAC414":{"n":101,"avg":5},"EWIE067-PWK":{"n":210,"avg":4.7},"CMC-A250":{"n":99,"avg":5},"MH67BR":{"n":54,"avg":4.9},"HS4E400-OWK":{"n":218,"avg":4.8},"AS527WA":{"n":56,"avg":5},"CMC-A210":{"n":6,"avg":4.8},"WT672NWR":{"n":40,"avg":4.9},"DT921":{"n":3,"avg":5},"CMC-A606":{"n":100,"avg":4.7},"WPU-JCC104":{"n":64,"avg":4.9},"SC5GMR80S":{"n":19,"avg":4.8},"CDW-D0620TWE":{"n":116,"avg":4.7},"MR775":{"n":7,"avg":5},"CFD-ENL201DCGW":{"n":212,"avg":4.8},"ACL-20C1A":{"n":2,"avg":5},"GBB011":{"n":12,"avg":4.8},"RH18WTSN":{"n":6,"avg":5},"SC5GMR60B":{"n":10,"avg":5},"AR107CBA":{"n":11,"avg":4.5},"AS305DWWA":{"n":24,"avg":5},"CHP-6340L":{"n":44,"avg":5},"BB15-A":{"n":22,"avg":4.8},"CHPI-5810L":{"n":17,"avg":5},"WP-60C90520M":{"n":1,"avg":4},"CMC-X2700":{"n":100,"avg":4.8},"WT652NWR":{"n":1,"avg":5},"WP-40C60000M":{"n":23,"avg":5},"OLED65G6KNA":{"n":18,"avg":5},"OLED55C5ENA":{"n":5,"avg":5},"OLED48C6KNA":{"n":19,"avg":5},"MH20BR":{"n":19,"avg":4.9},"GS5WK":{"n":59,"avg":4.9},"AS356NSMA":{"n":55,"avg":5},"MC-C02AT":{"n":2,"avg":5},"AS520HA":{"n":14,"avg":4.7},"ACL-130Z0SKPN":{"n":4,"avg":5},"ACL-16C":{"n":2,"avg":5},"ACL-25C1A":{"n":4,"avg":5},"ZB3511DB":{"n":100,"avg":4.8},"EFC52611":{"n":100,"avg":4.8},"BT660RWA":{"n":1,"avg":4},"AP-1623M":{"n":79,"avg":5},"MODUWAVE":{"n":6,"avg":5},"FX4WC":{"n":59,"avg":5},"WMG-2005":{"n":2,"avg":4.5},"EFP52216DG":{"n":100,"avg":4.8},"PC91-ALRG1":{"n":100,"avg":4.8},"HW500DAS":{"n":40,"avg":4.9},"WPUJCC104SWH":{"n":1,"avg":5},"WQ61-1OGG":{"n":100,"avg":4.8},"ZB6218STM":{"n":99,"avg":4.9},"PD91":{"n":98,"avg":4.9},"W087B":{"n":48,"avg":5},"DUB61TBE":{"n":11,"avg":4.8},"W0082GTB":{"n":12,"avg":4.9},"BAS27-D":{"n":32,"avg":4.9},"CHPI-390L":{"n":120,"avg":4.8},"DQ256MWGA":{"n":14,"avg":5},"DQ205PSVA":{"n":12,"avg":5},"WQ61-1EDB":{"n":99,"avg":4.8},"EFP52213SW":{"n":98,"avg":4.7},"EFP52212DB":{"n":99,"avg":4.8},"CDHC-200AXLWWYH":{"n":20,"avg":4.5},"CMS-A167":{"n":107,"avg":4.9},"WP-30S50010N":{"n":27,"avg":4.8},"65QNED70AEA":{"n":59,"avg":4.9},"P-2200N":{"n":60,"avg":4.9},"WI-55S90510M":{"n":27,"avg":5},"WI-60C90521M":{"n":5,"avg":5},"AD-1221E 셀프":{"n":47,"avg":4.9},"BB16-A":{"n":29,"avg":4.9},"OLED77B5SNA":{"n":19,"avg":5},"CIR-F41 셀프":{"n":1,"avg":5},"CIR-E301FW":{"n":120,"avg":4.9},"AP-2023K":{"n":79,"avg":5},"P-350N":{"n":58,"avg":5},"AP-1023F 셀프":{"n":3,"avg":5},"BB17-A":{"n":4,"avg":5},"WPUPBC204S 셀프형":{"n":20,"avg":4.9},"NW-BS150UW":{"n":74,"avg":4.6},"MR576(H) 6개월방문(프로모션)":{"n":1,"avg":5},"MR776(H) 6개월방문(프로모션)":{"n":5,"avg":5},"F9WTQ":{"n":3,"avg":5},"CP-AHS100HEW":{"n":1,"avg":4},"DRAGON-200":{"n":19,"avg":4.3},"VS28D950ACB":{"n":1,"avg":5},"MZ385EBT":{"n":6,"avg":5},"CRH-TWS1010W":{"n":59,"avg":4.8},"TS450":{"n":20,"avg":4.8},"NT940XJG-KC51G":{"n":1,"avg":5},"RQ33DG71J3EW":{"n":25,"avg":4.9},"AMH-9002":{"n":58,"avg":4.9},"KRECAM290.81.TB":{"n":18,"avg":4.9},"SL-C563W":{"n":56,"avg":4.7},"NH-A10XSB":{"n":5,"avg":5},"CWS-AO201W":{"n":1,"avg":5},"HY505RWLAH":{"n":45,"avg":4.9},"RBA120SS1MT9":{"n":6,"avg":4.7},"ECAM22":{"n":60,"avg":5},"Q8":{"n":60,"avg":4.9},"AP70F06103RTD":{"n":10,"avg":5},"AF60F17D11GS":{"n":56,"avg":4.8},"VS90F40CNG":{"n":14,"avg":4.8},"CRP-DHP0610FD":{"n":59,"avg":4.3},"RM70F90M1ZD":{"n":101,"avg":5},"HQPM15C4NE":{"n":20,"avg":4.8},"IK-A363CD":{"n":18,"avg":4.7},"HCW-6100":{"n":19,"avg":4.9},"IK-WR10":{"n":54,"avg":4.6},"OCP-BC200":{"n":20,"avg":4.7},"AF70F19D11GRS":{"n":100,"avg":4.7},"MNDW-110":{"n":23,"avg":4.7},"CIM-BS18M10NW":{"n":4,"avg":5},"W0082GCB":{"n":18,"avg":4.9},"DW80F75L1U01S":{"n":102,"avg":5},"DV90TA040TE":{"n":20,"avg":4.8},"KA-E1":{"n":9,"avg":4.9},"KA50":{"n":1,"avg":5},"SP-3":{"n":19,"avg":4.6},"CAP-100SW":{"n":16,"avg":4.8},"MNFD-200G":{"n":55,"avg":4.9},"OC-S1170S":{"n":1,"avg":5},"HY705RSUAB":{"n":38,"avg":4.9},"RFA-227E":{"n":10,"avg":4.2},"CDHC-200AXMWOYH":{"n":19,"avg":4.4},"KS148EG1N2":{"n":16,"avg":4.2},"AW06C7155GWAZ":{"n":17,"avg":5},"SL-J3260FW":{"n":3,"avg":4.7},"AP70F03102RTD":{"n":20,"avg":5},"AB-K8846RNF":{"n":20,"avg":4.8},"VR90F01AAG":{"n":58,"avg":4.9},"RH10WTW":{"n":102,"avg":5},"RQ33DG71J3S9":{"n":19,"avg":4.9},"MLJ32MRS 12개월":{"n":190,"avg":5},"CDHC-080AONWOYH":{"n":20,"avg":4.5},"RM70F90R2W":{"n":17,"avg":4.9},"HD-9100HTH":{"n":25,"avg":4.7},"H60 Hub Ultra":{"n":59,"avg":4.7},"TOA-60KR":{"n":73,"avg":4.9},"EW-210":{"n":5,"avg":5},"JK-DH07":{"n":1,"avg":5},"NT960XJG-KC51G":{"n":1,"avg":5},"VS28D950AIB":{"n":99,"avg":5},"RSB-260S":{"n":19,"avg":4.5},"RC-GT500W":{"n":20,"avg":4.8},"AY70H18100GVD":{"n":23,"avg":4.7},"HV-2318":{"n":1,"avg":5},"RC-GT500":{"n":20,"avg":4.9},"MNMD-120G":{"n":9,"avg":4.8},"WA16CG6441BY":{"n":54,"avg":4.9},"갤럭시탭S11 울트라 실버":{"n":59,"avg":4.9},"SQ06FA1WDS":{"n":24,"avg":4.9},"DW30FB305CW0":{"n":20,"avg":4.8},"AF-807":{"n":13,"avg":4.8},"SC5GMR81S":{"n":6,"avg":5},"EM-550KR":{"n":3,"avg":5},"OC-M2000PR":{"n":11,"avg":4.7},"DW30FB300CW0":{"n":20,"avg":4.8},"PQ08FDWBS":{"n":19,"avg":4.8},"FH25ESX":{"n":58,"avg":4.8},"RWEKG-C5":{"n":1,"avg":5},"DFFP-MAX100":{"n":60,"avg":4.8},"AMC-4500":{"n":20,"avg":4.7},"WPC-5900P":{"n":2,"avg":5},"SQ06EZ1WBS":{"n":3,"avg":5},"FP-30X":{"n":13,"avg":4.5},"FP-10":{"n":20,"avg":4.8},"PR35BL":{"n":16,"avg":5},"CFX5":{"n":3,"avg":5},"CL10RO":{"n":4,"avg":5},"PCM-F15":{"n":3,"avg":5},"AMC-5501A":{"n":20,"avg":4.8},"WD90H25AHS":{"n":3,"avg":4.7},"PQ07FDWCS":{"n":17,"avg":4.7},"WQP6-8204V1":{"n":5,"avg":4.2},"WQP6":{"n":10,"avg":4.5},"VS90F40CRG":{"n":20,"avg":4.8},"RM-L1379":{"n":5,"avg":3.8},"CDHC-200ACLWOYH":{"n":20,"avg":5},"CP-AMS100EPK":{"n":4,"avg":5},"BEI3ANHLOE 자가관리":{"n":101,"avg":5},"DWFP-2000":{"n":20,"avg":4.8},"KAE116TSME18":{"n":20,"avg":4.8},"GEN3":{"n":20,"avg":4.9},"FC1061AU":{"n":20,"avg":4.7},"ST212-B22":{"n":20,"avg":4.5},"PCM-NF15":{"n":2,"avg":5},"EDH10HNW":{"n":55,"avg":4.7},"H12 Pro FlexReach":{"n":20,"avg":4.2},"MNFD-120G":{"n":20,"avg":4.8},"FSDW211":{"n":1,"avg":5},"KS166EG1W1":{"n":11,"avg":4.8},"G5 S32DG500":{"n":2,"avg":5},"ADH-2000D":{"n":20,"avg":4.7},"NT960QHA-K71AR":{"n":60,"avg":4.9},"RC-EASY100":{"n":8,"avg":4.4},"LS27FG500S":{"n":20,"avg":4.8},"DR-100PRO":{"n":20,"avg":4.8},"BRAMS-K7G777BA":{"n":60,"avg":5},"LS32HG800":{"n":27,"avg":5},"D10 Plus Gen2":{"n":20,"avg":4.6},"EVO":{"n":20,"avg":5},"AT-210":{"n":3,"avg":5},"DH-Y1652FNW(셀프)":{"n":6,"avg":4.8},"DPF-104":{"n":20,"avg":4.7},"CRP-DHPNL0615FGW":{"n":22,"avg":4.7},"CRP-CHP1010FD":{"n":59,"avg":4.5},"AKAPSP082HRWW2":{"n":1,"avg":5},"VS-411P":{"n":1,"avg":5},"BN59-01259D":{"n":3,"avg":4.7},"DV21DG8600BE":{"n":20,"avg":4.8},"AY70H21100GTD":{"n":8,"avg":5},"VS-510M 셀프":{"n":1,"avg":5},"ST808":{"n":1,"avg":5},"BRAMS-K8G008BA":{"n":59,"avg":4.8},"WWRC051EEMWWO":{"n":20,"avg":4.7},"DV21DG8200BW":{"n":11,"avg":5},"CMS-G210NW":{"n":1,"avg":5},"DF24CB9900DR":{"n":13,"avg":5},"DF18CB8600DR":{"n":60,"avg":4.9},"TOA-70KR":{"n":57,"avg":4.9},"Q8 플러스":{"n":38,"avg":4.7},"RG20KN 12개월":{"n":116,"avg":5},"ECC-888":{"n":20,"avg":4.8},"HQPM15C20E":{"n":7,"avg":5},"NQ50T8539":{"n":9,"avg":5},"ND-A0610FM":{"n":20,"avg":5},"WQP6-8404Y1":{"n":2,"avg":5},"NT750XGR-A51A":{"n":57,"avg":4.9},"CRP-BDHF067FW":{"n":1,"avg":5},"HD-3000JZ7-3":{"n":20,"avg":5},"F25":{"n":60,"avg":4.5},"HUF65MDW":{"n":6,"avg":4.8},"EBDB111":{"n":4,"avg":4.8},"SC-S0201":{"n":20,"avg":4.5},"GX7192":{"n":5,"avg":5},"ADH-2000Y":{"n":20,"avg":4.6},"MF756CX":{"n":9,"avg":4.9},"AR06D1150HZS":{"n":60,"avg":4.5},"KQ55LSD01AFXKR":{"n":16,"avg":5},"AP90H10198UDD":{"n":7,"avg":5},"KMF-1":{"n":8,"avg":4.9},"MNMD-200G":{"n":1,"avg":5},"DCE118SWJ":{"n":14,"avg":4.9},"DE97-00136B":{"n":3,"avg":5},"KW-A09W1":{"n":3,"avg":4.7},"SC-D0404-A":{"n":8,"avg":4.8},"RH10WTA 12개월":{"n":112,"avg":5},"DSJ-N55P":{"n":1,"avg":5},"CRH-TWS0610W":{"n":60,"avg":4.6},"ABC-01":{"n":5,"avg":5},"AP0074AX":{"n":15,"avg":5},"ECAM350":{"n":59,"avg":4.9},"BAWB-0061YAWSD":{"n":8,"avg":4.9},"NID-0520T3":{"n":1,"avg":5},"NC-527":{"n":2,"avg":5},"GL-020K":{"n":5,"avg":5},"AC-14R10FNW(운영/4C)":{"n":1,"avg":5},"AP-1019D":{"n":2,"avg":5},"DV17CB6600BW":{"n":16,"avg":5},"DARB-0061GAWSD":{"n":20,"avg":5},"BRV07GWVH":{"n":10,"avg":4.5},"LK-5000":{"n":20,"avg":4.9},"DCP-580":{"n":39,"avg":5},"BEI3ASS4E 자가관리":{"n":71,"avg":5},"RH10VTA 12개월":{"n":100,"avg":4.9},"DR-300PRO":{"n":9,"avg":5},"SR-C65BS":{"n":6,"avg":5},"SR-C25AS":{"n":1,"avg":5},"MLJ32HWSO 12개월":{"n":172,"avg":5},"PCR-K100N":{"n":1,"avg":5},"BRAMS-K8G777SCR":{"n":20,"avg":4.8},"SMT-1000":{"n":18,"avg":4.5},"WPD-34":{"n":2,"avg":5},"WPD-18":{"n":5,"avg":4.8},"NDP-60":{"n":2,"avg":5},"IK-WR40":{"n":20,"avg":4.5},"RFA-328G":{"n":1,"avg":5},"IMK-3051":{"n":2,"avg":5},"AT-310":{"n":4,"avg":5},"EW60T18":{"n":5,"avg":5},"KDM-92":{"n":3,"avg":4.7},"SY-S1031":{"n":3,"avg":5},"DCP-580S":{"n":60,"avg":4.9},"HCW-1800A":{"n":1,"avg":5},"WPD-07":{"n":4,"avg":4.8},"N-0003H":{"n":20,"avg":4.8},"LMP-88":{"n":8,"avg":4.5},"WPC-1700":{"n":3,"avg":5},"AMB-350V":{"n":19,"avg":4.9},"PM10":{"n":7,"avg":5},"PWA-3500PB":{"n":16,"avg":4.9},"AR60F07D12WS":{"n":60,"avg":4.9},"KRECAM450.65.S":{"n":1,"avg":5},"BD-H720H0":{"n":18,"avg":4.9},"B6GA4PT":{"n":1,"avg":5},"TG03-0004KR":{"n":5,"avg":5},"KRECAM290":{"n":55,"avg":4.8},"AB-K6806NF":{"n":22,"avg":4.8},"OCS-RM700":{"n":20,"avg":3.6},"N-0003HF":{"n":20,"avg":4.5},"DUE5STE":{"n":18,"avg":4.9},"WPC-4900P":{"n":1,"avg":5},"BD-MCS4-03SB":{"n":2,"avg":5},"BD-AH70T":{"n":3,"avg":4.7},"DF18CG3100HR":{"n":4,"avg":5},"RG19VNS 12개월":{"n":155,"avg":5},"DF18CG3100TR":{"n":3,"avg":3.7},"KREC9255.M":{"n":1,"avg":5},"HWC35MNB":{"n":5,"avg":4.8},"HUF90MDB":{"n":7,"avg":4.7},"SC3GTE52 자가관리":{"n":13,"avg":5},"SC3GNE50 자가관리":{"n":17,"avg":4.9},"NT960QHA-KC51G":{"n":20,"avg":4.8},"CP-SS100HWS":{"n":59,"avg":4.7},"OCS-FS4000":{"n":20,"avg":4.8},"HCF198MDW":{"n":1,"avg":5},"SC-D0403":{"n":16,"avg":4.7},"RFA-M30G":{"n":1,"avg":5},"RFA-M20G":{"n":1,"avg":5},"AFC-451":{"n":18,"avg":4.8},"SS4RWS60E":{"n":112,"avg":4.9},"OCS-FS5800":{"n":3,"avg":5},"GL-005":{"n":6,"avg":4.5},"GL-015K":{"n":1,"avg":5},"SUS-2L1A":{"n":18,"avg":4.9},"AR60F09D11WS":{"n":54,"avg":5},"CAPF-V082HLW":{"n":2,"avg":5},"BEI3QEBLOE 자가관리":{"n":173,"avg":5},"VR7MD97716G":{"n":60,"avg":4.9},"VR30T85513W":{"n":1,"avg":5},"AF90H19D27SRS":{"n":5,"avg":5},"OLED65C6FNA":{"n":1,"avg":5},"DW30FB300CE0":{"n":19,"avg":4.7},"TOA-75KR":{"n":19,"avg":4.9},"VR90F01SAG":{"n":20,"avg":4.8},"NT960XJG-K72A":{"n":20,"avg":5},"MO22A7797CW2":{"n":20,"avg":5},"B182W13":{"n":125,"avg":5},"WPVW18EFPB":{"n":5,"avg":5},"PAF-4000W":{"n":1,"avg":5},"Apeos C325z":{"n":20,"avg":4.9},"L15160":{"n":5,"avg":5},"CRT-PWPD1010KFU":{"n":2,"avg":5},"CRT-WS0340TW":{"n":1,"avg":5},"CRH-VWZD0640KFNW":{"n":1,"avg":5},"RG18WNT 12개월":{"n":109,"avg":5},"MLJ39ER 12개월":{"n":76,"avg":4.8},"SS4RHS60E":{"n":104,"avg":5},"G5 S27CG550":{"n":20,"avg":5},"MLJ39KR 12개월":{"n":19,"avg":4.9},"MLJ32PWSO 12개월":{"n":80,"avg":5},"SC5MBR4ES 라이트":{"n":11,"avg":5},"BEI3QEBLOE":{"n":9,"avg":5},"MLJ39PR 12개월":{"n":8,"avg":5},"HY505RWLAHM":{"n":80,"avg":5},"DXTM120-NEK":{"n":20,"avg":4.6},"CP-F603":{"n":1,"avg":5},"DUB61TBE 12개월":{"n":18,"avg":4.7},"MLJ32MWSO 12개월":{"n":2,"avg":5},"65NA1C90AK":{"n":2,"avg":5},"FX-2000(탄력미인)":{"n":1,"avg":5},"SC5MSR42CS 라이트":{"n":2,"avg":5},"T20":{"n":1,"avg":5},"RSB-275S":{"n":17,"avg":4.8},"RSB-490S":{"n":20,"avg":4.6},"FX-2000(풀업미인)":{"n":7,"avg":4.7},"WR808":{"n":1,"avg":5},"BX9":{"n":1,"avg":5},"HA-M2192":{"n":1,"avg":5},"CP-ABS100GW":{"n":2,"avg":4.5},"CP-ABS100GWH":{"n":1,"avg":5},"CP-ABS100GP":{"n":2,"avg":5},"DW30FB305CE0":{"n":17,"avg":4.6},"1B15EG1WO":{"n":2,"avg":5}},"by_cat":{"일렉트로룩스|식기세척기":{"n":11,"avg":4.1},"코웨이|정수기":{"n":3163,"avg":4.9},"삼성|정수기":{"n":742,"avg":4.9},"로보락|로봇청소기":{"n":295,"avg":4.8},"LG|식기세척기":{"n":90,"avg":5},"에코백스|로봇청소기":{"n":464,"avg":4.8},"코웨이|공기청정기":{"n":423,"avg":4.9},"쿠쿠|청소기":{"n":220,"avg":4.8},"청호나이스|정수기":{"n":1075,"avg":4.9},"스마트카라|음식물처리기":{"n":499,"avg":4.8},"SK매직|정수기":{"n":1593,"avg":4.9},"LG|냉장고":{"n":177,"avg":5},"코지마|안마의자":{"n":811,"avg":4.8},"LG|정수기":{"n":111,"avg":5},"교원웰스|정수기":{"n":1488,"avg":4.9},"위닉스|공기청정기":{"n":1002,"avg":4.9},"위닉스|제습기":{"n":1526,"avg":4.8},"LG|공기청정기":{"n":225,"avg":5},"위닉스|건조기":{"n":732,"avg":4.8},"LG|청소기":{"n":282,"avg":4.9},"청호나이스|매트리스":{"n":34,"avg":4.9},"캐리어|에어컨":{"n":221,"avg":4.8},"위닉스|에어컨":{"n":695,"avg":4.7},"노비타|비데":{"n":30,"avg":4.8},"루헨스|정수기":{"n":63,"avg":5},"교원웰스|공기청정기":{"n":142,"avg":4.8},"블루에어|공기청정기":{"n":51,"avg":5},"캐리어|제습기":{"n":97,"avg":4.7},"쿠쿠|제습기":{"n":816,"avg":4.8},"쿠쿠|인덕션":{"n":323,"avg":4.8},"쿠쿠|제빙기":{"n":324,"avg":4.8},"LG|가습기":{"n":75,"avg":4.9},"로보락|청소기":{"n":130,"avg":4.7},"다이슨|청소기":{"n":306,"avg":4.6},"쿠쿠|비데":{"n":141,"avg":4.8},"쿠쿠|음식물처리기":{"n":929,"avg":4.7},"교원웰스|비데":{"n":280,"avg":4.9},"드리미|로봇청소기":{"n":99,"avg":4.5},"LG|세탁기":{"n":229,"avg":5},"쿠쿠|에어컨":{"n":106,"avg":4.5},"청호나이스|비데":{"n":230,"avg":4.9},"쿠쿠|공기청정기":{"n":120,"avg":4.8},"코웨이|비데":{"n":240,"avg":4.8},"다이슨|공기청정기":{"n":191,"avg":4.9},"LG|의류관리기":{"n":131,"avg":4.9},"코웨이|안마의자":{"n":3,"avg":5},"LG|건조기":{"n":114,"avg":5},"위닉스|청소기":{"n":28,"avg":4.8},"SK매직|공기청정기":{"n":114,"avg":4.9},"코웨이|제습기":{"n":51,"avg":4.9},"LG|안마의자":{"n":92,"avg":4.9},"다이슨|의류관리기":{"n":205,"avg":4.8},"교원웰스|제습기":{"n":3,"avg":5},"쿠쿠|식기세척기":{"n":120,"avg":4.7},"교원웰스|매트리스":{"n":13,"avg":5},"LG|제습기":{"n":74,"avg":5},"LG|인덕션":{"n":92,"avg":5},"코웨이|연수기":{"n":55,"avg":4.9},"LG|TV":{"n":61,"avg":5},"LG|노트북/태블릿":{"n":59,"avg":4.9},"일렉트로룩스|청소기":{"n":993,"avg":4.8},"교원웰스|이미용기":{"n":6,"avg":5},"코웨이|음식물처리기":{"n":2,"avg":4.5},"시몬스|매트리스":{"n":40,"avg":4.8},"드리미|청소기":{"n":13,"avg":4.9},"LG|TV·IT":{"n":59,"avg":4.9},"코웨이|인덕션":{"n":1,"avg":5},"쿠쿠|반려동물용품":{"n":74,"avg":4.6},"쿠쿠|정수기":{"n":1,"avg":4},"삼성|청소기":{"n":273,"avg":4.9},"LG|전기오븐/그릴":{"n":553,"avg":5},"쿠첸|전기밥솥/압력솥":{"n":123,"avg":4.7},"에브리봇|청소기":{"n":119,"avg":4.8},"삼성|노트북/태블릿":{"n":239,"avg":4.9},"삼성|냉장고":{"n":172,"avg":4.9},"에어메이드|가습기":{"n":58,"avg":4.9},"드롱기|커피머신":{"n":194,"avg":4.9},"삼성|프린터/스캐너":{"n":118,"avg":4.8},"쿠쿠|연수기":{"n":1,"avg":5},"라클라우드|매트리스":{"n":5,"avg":4.8},"쉐우드|소파":{"n":4,"avg":4.8},"위니아|냉장고":{"n":46,"avg":4.7},"삼성|공기청정기":{"n":37,"avg":5},"삼성|에어컨":{"n":358,"avg":4.8},"쿠쿠|전기밥솥/압력솥":{"n":160,"avg":4.4},"현대렌탈케어|반려동물용품":{"n":27,"avg":4.8},"인켈|TV·IT":{"n":92,"avg":4.6},"웰모아|안마의자":{"n":40,"avg":4.9},"싱크퓨어|주방가전":{"n":3,"avg":5},"오쿠|주방가전":{"n":49,"avg":4.8},"리코|프린터/스캐너":{"n":10,"avg":4.6},"아디다스|헬스기구":{"n":35,"avg":4.9},"쿠쿠|냉장고":{"n":24,"avg":4.6},"미닉스|식기세척기":{"n":23,"avg":4.7},"지엘플러스|주방가전":{"n":18,"avg":4.6},"린클|주방가전":{"n":108,"avg":4.8},"삼성|식기세척기":{"n":184,"avg":4.9},"삼성|건조기":{"n":78,"avg":4.9},"영창피아노|악기":{"n":10,"avg":4.9},"후지필름|프린터/스캐너":{"n":42,"avg":4.7},"캐리어|공기청정기":{"n":19,"avg":4.8},"미닉스|주방가전":{"n":75,"avg":4.8},"린나이|튀김기":{"n":13,"avg":4.4},"풀무원|냉장고":{"n":27,"avg":4.5},"SK매직|비데":{"n":16,"avg":4.2},"애플비데|비데":{"n":42,"avg":4.8},"휴렉|주방가전":{"n":45,"avg":4.8},"엡손|프린터/스캐너":{"n":125,"avg":4.9},"쿠진아트|전기오븐/그릴":{"n":152,"avg":4.9},"세스코|정수기":{"n":5,"avg":5},"제크롤|제습기":{"n":1,"avg":5},"린나이|인덕션":{"n":56,"avg":4.6},"삼성|제습기":{"n":47,"avg":4.9},"스포틀러|런닝머신":{"n":7,"avg":4.3},"아이스웰|제빙기":{"n":1,"avg":5},"미닉스|건조기":{"n":10,"avg":4.8},"삼성|세탁기":{"n":117,"avg":5},"LG|에어컨":{"n":63,"avg":4.8},"TAMA|청소기":{"n":80,"avg":4.9},"에어메이드|주방가전":{"n":50,"avg":4.8},"쿠진아트|커피머신":{"n":3,"avg":5},"오쿠|전기밥솥/압력솥":{"n":31,"avg":4},"캐논|프린터/스캐너":{"n":72,"avg":4.8},"MSI|노트북/태블릿":{"n":1,"avg":5},"DJI|청소기":{"n":17,"avg":4.9},"쉘퍼|주방가전":{"n":80,"avg":4.8},"에어메이드|청소기":{"n":40,"avg":4.7},"웰템|에어컨":{"n":6,"avg":5},"롤랜드|악기":{"n":33,"avg":4.7},"도메틱|냉장고":{"n":3,"avg":5},"리페르|냉장고":{"n":4,"avg":5},"플랜잇|커피머신":{"n":8,"avg":4.8},"쉐프본|식기세척기":{"n":17,"avg":4.5},"쿠쿠|살균/방역/향균":{"n":24,"avg":4.8},"HP|노트북/태블릿":{"n":21,"avg":4.7},"스팀보이|매트":{"n":20,"avg":4.5},"위니아|제습기":{"n":69,"avg":4.7},"나르왈|청소기":{"n":40,"avg":5},"세스코|살균/방역/향균":{"n":5,"avg":5},"삼성|모니터":{"n":148,"avg":4.9},"에어메이드|제습기":{"n":40,"avg":4.7},"페페|반려동물용품":{"n":29,"avg":4.8},"브람스|안마의자":{"n":139,"avg":4.9},"디클펫|반려동물용품":{"n":7,"avg":5},"비에스소닉|음파진동운동기":{"n":3,"avg":5},"싱크스타|주방가전":{"n":1,"avg":5},"김수자|매트리스":{"n":40,"avg":4.3},"세스코|공기청정기":{"n":1,"avg":5},"삼성|TV·IT":{"n":19,"avg":4.9},"스포틀러|헬스기구":{"n":16,"avg":4.8},"고고런|런닝머신":{"n":1,"avg":5},"쿠쿠|안마의자":{"n":1,"avg":5},"삼성|의류관리기":{"n":83,"avg":4.9},"한일의료기|매트":{"n":20,"avg":4.8},"홈쎄라|이미용기":{"n":13,"avg":5},"에코백스|청소기":{"n":32,"avg":4.8},"에코체|주방가전":{"n":41,"avg":4.9},"위더스컴퓨터|노트북/태블릿":{"n":6,"avg":5},"하만카돈|음향기기":{"n":61,"avg":4.9},"삼성|전기오븐/그릴":{"n":33,"avg":5},"하이얼|냉장고":{"n":19,"avg":4.8},"세스코|비데":{"n":4,"avg":4.8},"삼천리자전거|전기자전거":{"n":3,"avg":5},"펫말리|반려동물용품":{"n":2,"avg":5},"스마트카라|주방가전":{"n":44,"avg":4.6},"쿠쿠|주방가전":{"n":5,"avg":5},"귀뚜라미|매트":{"n":11,"avg":4.8},"코웨이|의류관리기":{"n":5,"avg":4.6},"쉐우드|매트리스":{"n":20,"avg":4.1},"오키멧|매트리스":{"n":14,"avg":4.6},"애플|노트북/태블릿":{"n":16,"avg":4.6},"대성하이원|제습기":{"n":1,"avg":5},"프리모|매트리스":{"n":26,"avg":4.9},"뉴젠|홈 사우나":{"n":5,"avg":5},"보탬큐브|이미용기":{"n":5,"avg":5},"핸슨코리아|매트리스":{"n":1,"avg":5},"HP|게이밍/고성능PC":{"n":20,"avg":5},"나까조|제빙기":{"n":1,"avg":5},"네오트|제빙기":{"n":2,"avg":5},"인홈|매트리스":{"n":1,"avg":5},"위니아|에어컨":{"n":15,"avg":4.7},"다이나톤|악기":{"n":99,"avg":4.9},"스타리온|냉장고":{"n":7,"avg":5},"붐펫|반려동물용품":{"n":1,"avg":5},"라클라우드|소파":{"n":1,"avg":5},"삼익|악기":{"n":20,"avg":4.5},"웰템|제습기":{"n":11,"avg":4.8},"카이저|제빙기":{"n":2,"avg":5},"엘바|냉장고":{"n":5,"avg":5},"셀리턴|이미용기":{"n":4,"avg":5},"누하스|안마의자":{"n":40,"avg":4.7},"럽마이펫|반려동물용품":{"n":8,"avg":4.5},"루컴즈|냉장고":{"n":21,"avg":4.6},"에쎈바이오|주방가전":{"n":12,"avg":5},"드리미|공기청정기":{"n":7,"avg":5},"파세코|에어컨":{"n":16,"avg":4.9},"플랜잇|전기오븐/그릴":{"n":1,"avg":5},"퓨라셀미인|이미용기":{"n":8,"avg":4.8},"툰투리|런닝머신":{"n":1,"avg":5},"고고런|헬스기구":{"n":2,"avg":5},"풀무원|제빙기":{"n":2,"avg":5}}}; // scripts/update-review-counts-snapshot.mjs가 매일 최신값으로 채운다.
  var BMAP = {'SK':'SK매직','웰스':'교원웰스','교원':'교원웰스','청호':'청호나이스','LG구독':'LG','현대렌탈케어':'현대큐밍'};
  ...
  function applyCounts(j, source){
    j = normalizeCounts(j);
    if(!j) return false;
    counts = j;
    counts._source = source || counts._source || 'api';
  ...
  function fetchCounts(){
    if(PRELOADED_COUNTS) applyCounts(PRELOADED_COUNTS, 'preloaded-' + PRELOADED_COUNTS_DATE);
  }
